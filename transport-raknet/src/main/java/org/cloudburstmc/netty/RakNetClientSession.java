...
public class RakNetClientSession extends RakSessionCodec {
    ...
    protected void tick(long curTime) {
        if (this.getState() == RakState.UNCONNECTED) {
    ...
    private void onOpenConnectionReply2(ByteBuf buffer) {
        ...
        this.initialize();
        this.setState(RakState.INITIALIZED);
