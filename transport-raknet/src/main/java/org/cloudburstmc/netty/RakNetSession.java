...
import org.cloudburst.netty.raknet.util.*;
import org.cloudburst.netty.util.*;
...
public abstract class RakNetSession implements SessionConnection<ByteBuf> {
    ...
    private ConcurrentMap<Integer, RakDatagramPacket> sentDatagrams;
    ...
    final void initialize() {
        Preconditions.checkState(this.state == RakState.INITIALIZING);
    ...
    void onDatagram(DatagramPacket datagram) {
        ...
        if (rakNetDatagram) {
            ...
            if (this.state.ordinal() >= RakState.INITIALIZED.ordinal()) {
    ...
    public void sendImmediate(ByteBuf buf) {
        this.send(buf, RakPriority.IMMEDIATE);
    }

    @Override
    public void send(ByteBuf buf) {
        this.send(buf, RakPriority.NORMAL);
    }

    public void send(ByteBuf buf, RakPriority priority) {
        this.send(buf, priority, RakReliability.RELIABLE_ORDERED);
    }

    public void send(ByteBuf buf, RakReliability reliability) {
        this.send(buf, RakPriority.NORMAL, reliability);
    }

    public void send(ByteBuf buf, RakPriority priority, RakReliability reliability) {
        this.send(buf, priority, reliability, 0);
    }

    public void send(ByteBuf buf, RakPriority priority, RakReliability reliability, @Nonnegative int orderingChannel) {
        this.sessionLock.readLock().lock();
        try {
            if (closed || state == null || state.ordinal() < RakState.INITIALIZED.ordinal()) {
    ...
    private EncapsulatedPacket[] createEncapsulated(ByteBuf buffer, RakPriority priority, RakReliability reliability,
                                                    int orderingChannel) {
    ...
    public RakState getState() {
        return state;
    }

    void setState(@Nullable RakState state) {
        if (this.state != state) {
            this.state = state;
            if (this.listener != null) {
