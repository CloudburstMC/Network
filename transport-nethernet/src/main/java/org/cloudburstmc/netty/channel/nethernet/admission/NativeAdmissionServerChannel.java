...
public final class NativeAdmissionServerChannel extends AbstractServerChannel {
    ...
    private static final class Session {
        ...
        Session(AdmissionGate.Reservation reservation, AdmittedNetherNetChildChannel child) { this.reservation = reservation; this.child = child; }
    }
    ...
    public NativeAdmissionServerChannel(NativeHostIdentity identity, AdmissionValidator validator, AdmissionGate.Limits limits, boolean allowWildcardBind) {
        this.identity = Objects.requireNonNull(identity); this.limits = Objects.requireNonNull(limits);
        gate = new AdmissionGate(limits, validator); this.allowWildcardBind = allowWildcardBind;
    }
    ...
    private CompletionStage<IceUdpMuxListener.Acceptance> admit(IceUdpMuxListener.Request request) throws Exception {
        if (!isOpen() || nativeCloseFailure.get() != null) return CompletableFuture.completedFuture(null);
        byte[] ip = NetUtil.createByteArrayFromIpAddressString(request.remoteAddress());
        if (ip == null) return CompletableFuture.completedFuture(null);
        ...
        if (reservation == null) return CompletableFuture.completedFuture(null);
        VerifiedAdmission a = gate.admission(reservation);
        CompletableFuture<Void> settled = new CompletableFuture<>(); admissions.add(settled);
            ...
            try {
                eventLoop().execute(() -> {
                    if (failure != null) {
                        ...
                        if (session == null) gate.finish(reservation);
                        else finish(reservation, "native_acceptance_failed");
                    }
                    settled.complete(null); admissions.remove(settled);
                });
            } catch (RejectedExecutionException stopped) {
                nativeCloseFailure.compareAndSet(null, stopped); gate.drain(); settled.completeExceptionally(stopped);
            }
        });
    ...
    private void initialize(AdmissionGate.Reservation reservation, VerifiedAdmission a, PeerConnection peer) {
        ...
        creations.incrementAndGet(); liveNativePeers.incrementAndGet();
        sessions.put(reservation, session); nativeClosures.add(session.closed);
        session.closed.whenComplete((ignored, failure) -> {
            if (failure != null) { nativeCloseFailure.compareAndSet(null, failure); gate.drain(); }
            else { liveNativePeers.decrementAndGet(); gate.finish(reservation); }
            nativeClosures.remove(session.closed);
        });
        child.nativeTermination().whenComplete((ignored, failure) -> {
            if (failure == null) session.closed.complete(null); else session.closed.completeExceptionally(failure);
        });
        ...
        peer.onDataChannel.register((p, dc) -> {
            ...
            try { child.acceptDataChannel(dc); }
            catch (Exception invalidChannel) { session.failed = true; }
        });
        if (!gate.ready(reservation)) throw new IllegalStateException("Admission cancelled");
    ...
    private void maintain() {
        ...
        try {
            ...
            if (listener != null && listener.failure() != null) nativeCloseFailure.compareAndSet(null, listener.failure());
            if (nativeCloseFailure.get() != null) { close(); return; }
    ...
    private void finish(AdmissionGate.Reservation r, String reason) {
        ...
        if (session != null) {
            closeChild(session);
            if (!session.reported) emit(r, "ticket.failed", reason, session.creationNanos);
        }
    }
    ...
    public long[] nativeStats() { IceUdpMuxListener listener = mux; if (listener == null) throw new IllegalStateException("Endpoint not bound"); return listener.stats(); }
    ...
    @Override protected void doClose() {
        ...
        IceUdpMuxListener listener = mux; mux = null;
        if (listener != null) listener.close();
        for (Session session : sessions.values()) closeChild(session);
        sessions.clear();
        List<CompletableFuture<Void>> outstanding = new ArrayList<>(nativeClosures); outstanding.addAll(admissions);
