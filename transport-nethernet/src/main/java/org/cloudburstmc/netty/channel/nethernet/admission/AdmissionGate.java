...
public final class AdmissionGate {
    ...
    public record Stats(int sessions, int pending, int claims, long invalid, long replayRejected, long capacityRejected, long accepted) {}
    ...
    public AdmissionGate(Limits limits, AdmissionValidator validator) {
        this.limits = Objects.requireNonNull(limits); this.validator = Objects.requireNonNull(validator);
    }
    ...
    public synchronized Reservation reserve(AdmissionRequest request, long nowMillis, long nowNanos) {
        ...
        if (a == null) { invalid++; return null; }
        if (claims.containsKey(a.tokenId()) || tuples.containsKey(request.address())) { replayRejected++; return null; }
        if (draining) { capacityRejected++; return null; }
        if (pending >= limits.pending()) {
            capacityRejected++; pendingLimitRejected++; pendingAtRejection = pending;
            return null;
        }
        if (tuples.size() >= limits.sessions() || claims.size() >= limits.claims()) { capacityRejected++; return null; }
        Reservation r = new Reservation(a, request.address(), nowNanos);
        claims.put(r.tokenId, r); tuples.put(r.tuple, r); pending++;
        return r;
    }

    public synchronized VerifiedAdmission admission(Reservation r) { return current(r) && !r.closing ? r.admission : null; }
    ...
    public synchronized boolean ready(Reservation r) {
        if (!current(r) || r.closing || r.ready) return false;
        r.ready = true; pending--; accepted++;
        return true;
    }
    public synchronized void connected(Reservation r) { if (current(r) && !r.closing) r.connected = true; }
    public synchronized void invalidNativeRequest() { invalid++; }
    ...
    public synchronized boolean finish(Reservation r) {
        ...
        if (!r.ready) pending--;
        tuples.remove(r.tuple); r.closed = true; r.admission = null;
        // A copied token with forged STUN integrity must not consume the real client's token.
        if (!r.ready || closed) claims.remove(r.tokenId);
        return true;
    }
    ...
    public synchronized List<Reservation> sweep(long nowMillis, long nowNanos) {
        ...
        for (Reservation r : claims.values()) {
            if (!r.closed && !r.closing && !r.connected && nowNanos - r.acceptedNanos >= limits.handshakeMillis() * 1_000_000L) {
                r.closing = true; timedOut.add(r);
            }
        }
    ...
    public synchronized List<Reservation> close() {
        ...
        for (Reservation r : active) r.closing = true;
        claims.values().removeIf(r -> r.closed);
        return active;
    }
    public synchronized Stats stats() { return new Stats(tuples.size(), pending, claims.size(), invalid, replayRejected, capacityRejected, accepted); }
