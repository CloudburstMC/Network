...
public final class EndpointSelection {
    ...
    public static EndpointSelection select(
            InetSocketAddress bind,
            List<InetSocketAddress> configured,
            List<Candidate> localHints) {
        requireEndpoint(bind);
        if (bind.getAddress().isMulticastAddress()) {
            throw new IllegalArgumentException("Unicast or wildcard bind required");
        }
        Objects.requireNonNull(configured, "configured");
        Objects.requireNonNull(localHints, "localHints");
        Set<Family> families =
                bind.getAddress() instanceof Inet6Address && bind.getAddress().isAnyLocalAddress()
                        ? EnumSet.allOf(Family.class)
                        : EnumSet.of(Family.of(bind.getAddress()));
            ...
            for (InetSocketAddress endpoint : configured) {
                ...
                if (!EndpointAddress.advertisable(endpoint.getAddress(), false)) {
                    throw new IllegalArgumentException(
                            "Configured endpoint must be a usable unicast address");
                }
                result.putIfAbsent(endpoint, new Candidate(endpoint, Provenance.CONFIGURED));
            }
        } else {
            if (localHints.size() > 256) {
                throw new IllegalArgumentException("At most 256 local discovery hints");
            }
            addLocal(result, new Candidate(bind, Provenance.LOCAL_BIND), bind, families);
            // Fixed order makes duplicate provenance stable when interface enumeration order
            // changes.
            localHints.stream()
                    .sorted(Comparator.comparing(Candidate::provenance))
                    .forEach(
                            hint -> {
                                if (hint.provenance() == Provenance.CONFIGURED
                                        || hint.provenance() == Provenance.LOCAL_BIND) {
                                    throw new IllegalArgumentException(
                                            "Local hints cannot claim configured or bound provenance");
                                }
                                addLocal(result, hint, bind, families);
                            });
        }
        if (result.size() > 32) {
            throw new IllegalArgumentException("At most 32 selected endpoints");
        }
                ...
                result.values().stream()
                        .sorted(
                                Comparator.comparing(
                                                (Candidate c) ->
                                                        Family.of(c.endpoint().getAddress()))
                                        .thenComparing(
                                                c -> c.endpoint().getAddress().getHostAddress())
                                        .thenComparingInt(c -> c.endpoint().getPort()))
                        .toList();
    ...
    public static EndpointSelection discover(
            InetSocketAddress bind, List<InetSocketAddress> configured, List<Candidate> hostHints)
            throws IOException {
        requireEndpoint(bind);
        ...
        if (networks != null) {
            for (NetworkInterface network : Collections.list(networks)) {
                ...
                for (InetAddress address : Collections.list(network.getInetAddresses())) {
                    if (hints.size() == 256) {
                        throw new IOException("Too many local discovery hints");
                    }
                    hints.add(
                            new Candidate(
    ...
    private static void addLocal(
            Map<InetSocketAddress, Candidate> result,
            Candidate candidate,
            InetSocketAddress bind,
            Set<Family> families) {
        if (candidate.endpoint().getPort() != bind.getPort()) {
            throw new IllegalArgumentException(
                    "Local candidate must use the fixed gameplay mux port");
        }
        if (EndpointAddress.advertisable(candidate.endpoint().getAddress(), false)
            ...
            // A concrete socket only owns that interface; other interfaces are not listener
            // candidates.
            if (!bind.getAddress().isAnyLocalAddress()
                    && candidate.provenance() == Provenance.LOCAL_INTERFACE
                    && !candidate.endpoint().getAddress().equals(bind.getAddress())) {
                return;
            }
    ...
    public Set<Family> socketFamilies() {
        return socketFamilies;
    }
    ...
    public List<Candidate> publicCandidates(Family family) {
        return candidates(family).stream()
                .filter(
                        c ->
