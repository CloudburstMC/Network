...
public final class ControlOrigin {
    ...
    public static boolean isCanonical(String value) {
        ...
        if (!secure && !value.startsWith("http://")) {
            return false;
        }
        String authority = value.substring(secure ? 8 : 7);
        if (authority.isEmpty() || !authority.matches("[a-z0-9.\\[\\]:-]+")) {
            return false;
        }
        String host, port;
        if (authority.startsWith("[")) {
            ...
            if (!ipv6(host.substring(1, host.length() - 1))) {
                return false;
            }
        } else {
        ...
        if (!port.isEmpty()) {
            ...
            if (number > 65535 || number == (secure ? 443 : 80)) {
                return false;
            }
        }
        return secure
    ...
    public static void requireCanonical(String value) {
        if (!isCanonical(value)) {
            throw new IllegalArgumentException("Invalid canonical control origin");
        }
    }
    ...
    private static boolean dns(String host) {
        ...
        return labels[labels.length - 1].charAt(0) >= 'a'
                && labels[labels.length - 1].charAt(0) <= 'z';
    }

    private static boolean ipv6(String input) {
        if (input.isEmpty() || !input.matches("[0-9a-f:]+")) {
            return false;
        }
        int compression = input.indexOf("::");
        if (compression != input.lastIndexOf("::")) {
            return false;
        }
        List<Integer> pieces = new ArrayList<>(8);
        if (compression < 0) {
            if (!pieces(input, pieces) || pieces.size() != 8) {
                return false;
            }
        } else {
            List<Integer> right = new ArrayList<>(8);
            if (!pieces(input.substring(0, compression), pieces)
                    || !pieces(input.substring(compression + 2), right)) {
                return false;
            }
        ...
        for (int i = 0; i < pieces.size(); ) {
            ...
            int start = i;
            while (i < pieces.size() && pieces.get(i) == 0) {
                i++;
            }
        ...
        String canonical;
        if (bestStart < 0) {
            canonical = join(pieces, 0, 8);
        } else {
            canonical = join(pieces, 0, bestStart) + "::" + join(pieces, bestStart + bestLength, 8);
        }
