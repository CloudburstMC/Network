...
final class ProviderWebSocket implements AutoCloseable {
    ...
    static final List<String> AUTH_HEADERS =
            List.of(
                    "nxs-instance-id",
                    "nxs-key-id",
                    "nxs-timestamp",
                    "nxs-signature-version",
                    "nxs-generation",
                    "nxs-sequence",
                    "idempotency-key",
                    "nxs-signature");
    ...
    private static final JdkWebSocketTransport.Limits LIMITS =
    ...
    // Separate from the serialized/blocking ProviderClient executor, including during open and
    // receive.
    ...
    Reply exchange(
        ...
        for (String name : AUTH_HEADERS) {
            headers.addProperty(name, request.headers().firstValue(name).orElseThrow());
        }
        frame.addProperty("operation", operation);
        frame.add("headers", headers);
        frame.addProperty("body", body);
        String wire = JSON.toJson(frame);
        if (wire.getBytes(StandardCharsets.UTF_8).length > MAX_FRAME_BYTES) {
            return null;
        }
        String binding =
                headers.get("nxs-instance-id").getAsString()
                        + ":"
                        + headers.get("nxs-generation").getAsString();
        Connection connection;
        synchronized (this) {
            if (closed) {
                throw new IOException("WebSocket carrier is closed");
            }
            if (current != null && !binding.equals(current.binding)) {
            ...
            if (current == null) {
                ...
                connection = new Connection(binding);
                current = connection;
        ...
        try {
            ...
            synchronized (this) {
                if (current != connection) {
                    throw new IOException("WebSocket connection changed");
                }
                if (connection.pending != null) {
                    throw new IOException("Concurrent provider operation");
                }
                connection.pending = reply;
                connection.id = headers.get("idempotency-key").getAsString();
            }
            // Executed on ProviderClient's serialized thread before any operation frame can be
            // sent.
            beforeSend.persist();
            connection
                    .transport
                    .sendText(wire, requireCurrent)
                    .whenComplete(
                            (ignored, failure) -> {
                                if (failure != null) {
    ...
    private static Parsed parse(String wire) throws IOException {
        try (JsonReader reader = new JsonReader(new StringReader(wire))) {
            ...
            while (reader.hasNext()) {
                ...
                if (!fields.add(name)) {
                    throw new IOException("Duplicate response field");
                }
                switch (name) {
                    case "id" -> {
                        id = string(reader);
                        if (!id.matches("[A-Za-z0-9_-]{16,128}")) {
                    ...
                    case "headers" -> {
                        ...
                        while (reader.hasNext()) {
                            ...
                            if (value.length() > 1024
                                ...
                                throw new IOException("Invalid response header value");
                            }
                    ...
                    default -> {
                        throw new IOException("Unknown response field");
                    }
                }
            }
            reader.endObject();
            if (!fields.equals(Set.of("id", "status", "headers", "body"))
                    || reader.peek() != JsonToken.END_DOCUMENT) {
                throw new IOException("Incomplete response envelope");
            }
            return new Parsed(
                    id, new Reply(status, HttpHeaders.of(headers, (name, value) -> true), body));
    ...
    private static String string(JsonReader reader) throws IOException {
        if (reader.peek() != JsonToken.STRING) {
            throw new IOException("Expected response string");
        }
    ...
    private synchronized void keepAlive() {
        ...
        if (closed
                || connection == null
                || !connection.transport.opened().toCompletableFuture().isDone()) {
            return;
        }
        if (connection.pingAt != 0) {
            discard(connection, true);
            return;
        }
    ...
    private synchronized void lost(Connection connection) {
        discard(connection, true);
    }
    ...
    private void discard(Connection connection, boolean backoff) {
        ...
        if (backoff) {
            ...
            failures++;
            long delay = Math.min(30000, 500L << Math.min(previousFailures, 6));
            retryAt =
                    System.nanoTime()
        ...
        if (connection.pending != null) {
            connection.pending.completeExceptionally(
                    new IOException("Provider WebSocket disconnected"));
        }
