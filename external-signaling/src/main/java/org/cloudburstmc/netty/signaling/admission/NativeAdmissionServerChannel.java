...
public final class NativeAdmissionServerChannel extends AbstractServerChannel {
    ...
    public record Event(
            ...
            String reason,
            long occurredAt,
            long validationToCreationNanos,
    ...
    private static final class Session {
        ...
        Session(
                ...
                AdmittedNetherNetChildChannel child,
                PeerConnection peer) {
    ...
    private CompletionStage<IceUdpMuxListener.Acceptance> admit(IceUdpMuxListener.Request request) throws Exception {
        ...
        if (known != null) {
            ...
            try {
                ...
                if (admission == null
                        || System.nanoTime() >= known.deadlineNanos()
                        || System.currentTimeMillis() >= admission.expiresAt()
                        || !admission.remoteUfrag().equals(request.remoteUfrag())) {
                    return CompletableFuture.completedFuture(null);
                }
                return CompletableFuture.completedFuture(
                        IceUdpMuxListener.Acceptance.reuse(
                                known.peer(), Instant.ofEpochMilli(admission.expiresAt())));
        ...
        if (diagnostic != null) {
            ...
            if (players.sessions() + checks.active() >= limits.sessions()
                    || players.pending() + checks.pending() >= limits.pending()) {
                return CompletableFuture.completedFuture(null);
            }
        }
        ...
        // Purpose comes only from the authenticated token. Zero can never reach the player
        // pipeline.
    ...
    public CompletionStage<String> assistDiagnostic(AssistedJoin join, Runnable requireCurrent) {
        return assistDiagnostic(
                join,
                requireCurrent,
                Map.of(),
                Map.of(address.getAddress() instanceof Inet6Address ? 6 : 4, address));
    }

    public CompletionStage<String> assistDiagnostic(
            AssistedJoin join,
            Runnable requireCurrent,
            Map<Integer, InetSocketAddress> stunServers,
            Map<Integer, InetSocketAddress> publicCandidates) {
        var servers = Map.copyOf(stunServers);
        var candidates = Map.copyOf(publicCandidates);
                ...
                .execute(
                        () -> {
                            try {
                                ...
                                if (!isServing() || gate == null) {
                                    throw new IllegalStateException(
                                            "Diagnostic assistance unavailable");
                                }
                                var players = this.gate.stats();
                                var checks = gate.stats();
                                if (players.sessions() + checks.active() >= limits.sessions()
                                        || players.pending() + checks.pending()
                                                >= limits.pending()) {
                                    throw new IllegalStateException(
                                            "Diagnostic capacity unavailable");
                                }
                                String description = gate.assist(join, requireCurrent);
                                gatherAssistedAnswer(
                                        join,
                                        description,
                                        () -> {
                                            requireCurrent.run();
                                            gate.requireAssistedCurrent(join);
                                        },
                                        () -> gate.cancelAssisted(join),
                                        candidate -> gate.assistedAnswerCandidate(join, candidate),
                                        servers,
                                        candidates,
                                        result);
    ...
    CompletionStage<String> assist(
        ...
        if (remaining <= 0 || remaining > 30_000) {
            return CompletableFuture.failedFuture(
                    new IllegalArgumentException("Assisted deadline"));
        }
                            ...
                            try {
                                ...
                                if (join.diagnostic()) {
                                    throw new IllegalArgumentException(
                                            "Diagnostic purpose requires diagnostic gate");
                                }
                                if (!isServing()
                                        ...
                                        || assisted.size() >= 32
                                        || assisted.containsKey(join.localUfrag())) {
                                    throw new IllegalStateException(
                                            "Assisted admission unavailable");
                                }
                                        ...
                                        java.util.HexFormat.of()
                                                .formatHex(
                                                        MessageDigest.getInstance("SHA-256")
                                                                .digest(cpk));
                                IdentityKeyVerifier verifier =
                                        new IdentityKeyVerifier() {
                                            ...
                                            protected boolean matches(byte[] actual) {
                                                return MessageDigest.isEqual(cpk, actual);
                                            }

                                            protected void release() {
                                                Arrays.fill(cpk, (byte) 0);
                                            }
                                        };
                                AdmissionContext admission =
                                        new AdmissionContext() {
                                            ...
                                            public String localUfrag() {
                                                return join.localUfrag();
                                            }

                                            public String localPassword() {
                                                return join.localPassword();
                                            }
                                            ...
                                            public String identityBindingHex() {
                                                return identityHash;
                                            }
                                            ...
                                            public IdentityKeyVerifier identityVerifier() {
                                                return verifier;
                                            }
                                        };
                                reservation =
                                ...
                                peer =
                                        PeerConnection.createPeer(
                                                PeerConnectionConfiguration.DEFAULT
                                                        .withBindAddress(address.getAddress())
                                                        .withEnableIceUdpMux(true)
                                                        .withPortRangeBegin(address.getPort())
                                                        .withPortRangeEnd(address.getPort())
                                                        .withDisableAutoNegotiation(true)
                                                        .withMaxMessageSize(
                                                                NetherNetFrameDecoder
                                                                        .MESSAGE_LIMIT),
                                                Runnable::run,
                                                new tel.schich.libdatachannel.DtlsIdentity(
                                                        identity.certificate(),
                                                        identity.privateKey()));
                                assisted.put(
                                        join.localUfrag(),
                                        new AssistedPeer(
                                                peer, reservation, deadline, requireCurrent));
                                peer.setRemoteDescription(
                                        join.offer(),
                                        tel.schich.libdatachannel.SessionDescriptionType.OFFER);
                                peer.setLocalDescription(
                                        "answer", join.localUfrag(), join.localPassword());
                                initialize(reservation, admission, peer);
                                requireCurrent.run();
                                if (System.nanoTime() >= deadline
                                        || System.currentTimeMillis() >= join.expiresAt()) {
                                    throw new IllegalStateException("Assisted deadline");
                                }
                                ...
                                gatherAssistedAnswer(
                                        ...
                                        () -> {
                                            ...
                                            if (System.nanoTime() >= deadline
                                                    || System.currentTimeMillis()
                                                            >= join.expiresAt()
                                                    || !sessions.containsKey(ownedReservation)
                                                    || gate.admission(ownedReservation) == null) {
                                                throw new IllegalStateException(
                                                        "Assisted peer retired");
                                            }
                                        },
                                        () -> finish(ownedReservation, "assisted_answer_failed"),
                                        candidate -> {},
                                        servers,
                                        candidates,
                                        result);
                                ...
                                if (owned != null && owned.peer() == peer) {
                                    assisted.remove(join.localUfrag());
                                }
                                if (reservation != null) {
                                    if (sessions.containsKey(reservation)) {
                                        finish(reservation, "assisted_failed");
                                    } else {
                                        if (peer != null
    ...
    private void gatherAssistedAnswer(
            AssistedJoin join,
            String description,
            Runnable requireCurrent,
            Runnable cancelPeer,
            java.util.function.Consumer<InetSocketAddress> capture,
            Map<Integer, InetSocketAddress> servers,
            Map<Integer, InetSocketAddress> candidates,
            CompletableFuture<String> result) {
        StunUdpMuxMonitor monitor = null;
        try {
            requireCurrent.run();
            long started = System.nanoTime(),
                    remaining = join.expiresAt() - System.currentTimeMillis();
            if (remaining <= 0
                    || gatheringAnswers.size() >= 36
                    || servers.size() > 2
                    || candidates.size() > 2) {
                throw new IllegalStateException("Assisted gathering unavailable");
            }
            var families = new LinkedHashSet<Integer>();
            AssistedJoin.parseOffer(join.offer())
                    .candidates()
                    .forEach(c -> families.add(c.getAddress() instanceof Inet6Address ? 6 : 4));
            for (int family : families) {
                ...
                if (candidate != null) {
                    ...
                    requireCurrent.run();
                    if (System.currentTimeMillis() >= join.expiresAt()) {
                            ...
                            .orElseThrow(
                                    () ->
                                            new IllegalStateException(
                                                    "No eligible assisted discovery endpoint"));
    ...
    private void failAssistedAnswer(
            StunUdpMuxMonitor monitor,
            Runnable cancelPeer,
            CompletableFuture<String> result,
            Throwable failure) {
        try {
            closeAssistedMonitor(monitor);
        ...
        try {
            ...
            nativeCloseFailure.compareAndSet(null, cleanup);
            if (cleanup != failure) {
                failure.addSuppressed(cleanup);
            }
        }
    ...
    private void requireAnswerEndpoint(InetSocketAddress endpoint, int family) {
        if (endpoint.isUnresolved()
                || endpoint.getPort() < 1
                || (endpoint.getAddress() instanceof Inet6Address ? 6 : 4) != family
                || (EndpointAddress.scope(endpoint.getAddress()) != EndpointAddress.Scope.PUBLIC
                        && !(address.getAddress().isLoopbackAddress()
                                && endpoint.getAddress().isLoopbackAddress()))) {
            throw new IllegalArgumentException(
                    "Assisted discovery requires an eligible numeric endpoint");
        }
    }

    private static String assistedAnswer(
            String description, InetSocketAddress candidate, String type) {
        return String.join(
                        "\r\n",
                        description
                                .lines()
                                .filter(
                                        line ->
                                                !line.startsWith("a=candidate:")
                                                        && !line.startsWith("a=remote-candidates:")
                                                        && !line.equals("a=end-of-candidates"))
                                .toList())
    ...
    private void finishAssistedAnswers() {
        for (AssistedAnswer pending : new ArrayList<>(gatheringAnswers)) {
            try {
                ...
                if (!isOpen()
                        || pending.result().isCancelled()
                        || System.nanoTime() >= pending.deadlineNanos()
                        || System.currentTimeMillis() >= pending.join().expiresAt()) {
                    throw new IllegalStateException("Assisted gathering expired");
                }
                var observation = pending.monitor().binding(0);
                if (observation.isEmpty()
                        || observation.get().state() != StunBinding.State.SUCCEEDED) {
                    continue;
                }
                var mapping = observation.get();
                InetSocketAddress candidate =
                        new InetSocketAddress(
                                EndpointAddress.parse(mapping.mappedAddress()),
                                mapping.mappedPort());
                ...
                pending.requireCurrent().run();
                if (System.nanoTime() >= pending.deadlineNanos()
                        || System.currentTimeMillis() >= pending.join().expiresAt()) {
                    throw new IllegalStateException("Assisted gathering expired");
                }
                ...
                gatheringAnswers.remove(pending);
                failAssistedAnswer(
                        pending.monitor(), pending.cancelPeer(), pending.result(), failure);
            }
        }
    }
    ...
    private void initialize(
        ...
        if (a instanceof VerifiedAdmission) {
    ...
    private void maintain() {
        ...
        try {
            ...
            if (diagnostics != null) {
                diagnostics.tick();
                if (diagnostics.failure() != null) {
                    nativeCloseFailure.compareAndSet(null, diagnostics.failure());
                    close();
                    return;
                }
            }
                ...
                if (!session.reported && session.child.isActive()) {
                    if (assisted.values().removeIf(p -> p.reservation() == session.reservation)) {
                        emit(
                                session.reservation,
                                "ticket.ice_seen",
                                "assisted_ice_dtls_established",
                                session.creationNanos);
                    }
    ...
    private static InetSocketAddress observedRemoteEndpoint(PeerConnection peer) {
        try {
            ...
            if (remote.getPort() < 1 || remote.getPort() > 65535) {
                return null;
            }
    ...
    public int liveNativePeers() {
        ...
        return liveNativePeers.get()
                + (diagnostic == null ? 0 : diagnostic.stats().liveNativePeers());
    }
     ...
     */
    public CompletionStage<EndpointConnectivityController> enableConnectivity(
                    ...
                    .execute(
                            () -> {
                                try {
                                    if (!isActive()
                                            || connectivity != null
                                            || !selection.bind().equals(address)) {
                                        throw new IllegalStateException(
                                                "Connectivity requires the same active mux and one controller");
                                    }
                                    IceUdpMuxListener listener = mux;
                                    connectivity =
                                            new EndpointConnectivityController(
                                                    ...
                                                    server -> {
                                                        StunUdpMuxMonitor monitor =
                                                                listener.monitorStun(
                                                                        server.getAddress()
                                                                                .getHostAddress(),
                                                                        server.getPort());
                                                        return new EndpointConnectivityController
                                                                .Monitor() {
                                                            @Override
                                                            public Optional<
                                                                                    ...
                                                                                    try {
                                                                                        ...
                                                                                        return new EndpointConnectivityController
                                                                                            ...
                                                                                            UnknownHostException
                                                                                                    invalid) {
                                                                                        throw new IllegalStateException(
                                                                                                "Native STUN observation is not numeric",
                                                                                                invalid);
                                                                                    }
                                                                                });
                                                            }
    ...
    public CompletionStage<NativeDiagnosticHostGate> enableDiagnostics(
        ...
        try {
            eventLoop()
                    .execute(
                            () -> {
                                try {
                                    if (!isActive() || diagnostics != null) {
                                        throw new IllegalStateException(
                                                "Diagnostic gate requires one active host listener");
                                    }
                                    diagnostics =
                                            new NativeDiagnosticHostGate(identity, address, policy);
    ...
    NativeDiagnosticHostGate installDiagnostics(
            DiagnosticHostPolicy policy, NativeDiagnosticHostGate expected) {
        ...
        if (diagnostics == null) {
            ...
            diagnostics.replacePolicy(policy);
        }
        return diagnostics;
    }
    ...
    protected void doClose() {
        ...
        finishAssistedAnswers(); // isOpen=false closes every per-attempt monitor before mux
        // retirement.
        if (diagnostics != null) {
            diagnostics.close();
        }
        ...
        if (diagnostics != null) {
            outstanding.add(diagnostics.termination().toCompletableFuture());
        }
