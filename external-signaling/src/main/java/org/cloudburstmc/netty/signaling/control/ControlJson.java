...
final class ControlJson {
    ...
    static JsonObject parse(String wire, int maximum) {
        if (wire == null
                || wire.length() > maximum
                || wire.getBytes(StandardCharsets.UTF_8).length > maximum) {
            throw invalid("size");
        }
        if (wire.startsWith("\uFEFF")) {
            throw invalid("JSON byte order mark");
        }
        try (JsonReader reader = new JsonReader(new StringReader(wire))) {
            ...
            if (!value.isJsonObject() || reader.peek() != JsonToken.END_DOCUMENT) {
                throw invalid("object or trailing data");
            }
        ...
        return switch (reader.peek()) {
            case BEGIN_OBJECT -> {
                ...
                while (reader.hasNext()) {
                    ...
                    if (names.size() >= 32 || !names.add(name)) {
                        throw invalid("duplicate field or object size");
                    }
            ...
            case STRING -> {
                yield new JsonPrimitive(reader.nextString());
            }
            case NUMBER -> {
                String token = reader.nextString();
                if (!token.matches("0|[1-9][0-9]{0,15}")) {
                    throw invalid("integer token");
                }
            ...
            case BOOLEAN -> {
                yield new JsonPrimitive(reader.nextBoolean());
            }
            case NULL -> {
                reader.nextNull();
    ...
    static void fields(JsonObject object, String... expected) {
        if (!object.keySet().equals(Set.of(expected))) {
    ...
    static void safe(long number, boolean positive) {
        if (number < (positive ? 1 : 0) || number > MAX_SAFE_INTEGER) {
            throw invalid("safe integer");
        }
    }
