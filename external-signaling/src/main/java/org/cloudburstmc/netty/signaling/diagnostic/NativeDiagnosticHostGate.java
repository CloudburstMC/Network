...
public final class NativeDiagnosticHostGate implements AutoCloseable {
    public record Result(
            ...
            long expiresAt,
            DiagnosticHostPolicy.Endpoint target,
            boolean success,
            String reason,
            InetSocketAddress selectedLocal,
            InetSocketAddress selectedRemote,
            int sentFrames,
            int sentBytes,
            int receivedFrames,
            int receivedBytes,
    ...
    public NativeDiagnosticHostGate(
            NativeHostIdentity identity,
            InetSocketAddress listenerAddress,
            DiagnosticHostPolicy policy) {
        this(identity, listenerAddress, policy, Clock.system());
    }

    NativeDiagnosticHostGate(
            NativeHostIdentity identity,
            InetSocketAddress listenerAddress,
            DiagnosticHostPolicy policy,
            Clock clock) {
        ...
        if (listenerAddress.isUnresolved() || listenerAddress.getPort() == 0) {
            throw invalid();
        }
        this.listenerAddress = listenerAddress;
        if (observe() >= policy.expiresAt()) {
            throw invalid();
        }
    }
    ...
    public synchronized void replacePolicy(DiagnosticHostPolicy replacement) {
        ...
        if (closed
                || !replacement.context().hostId().equals(policy.context().hostId())
                || !replacement.context().providerOrigin().equals(policy.context().providerOrigin())
                || observe() >= replacement.expiresAt()) {
            throw invalid();
        }
        policy = replacement;
        for (Session session : new ArrayList<>(sessions.values())) {
            if (!authorized(session, observe())) {
                stop(session, "authority_changed");
            }
        }
    }
    ...
    public synchronized Stats stats() {
        ...
        return new Stats(
                sessions.size(), pending, used.size(), nativePeers, rejected, droppedResults);
    }
    ...
    public List<Result> pollResults() {
        return pollResults(32);
    }

    public List<Result> pollResults(int maximum) {
        if (maximum < 0 || maximum > 32) {
            throw new IllegalArgumentException("Diagnostic result poll bound");
        }
        List<Result> out = new ArrayList<>(maximum);
        results.drainTo(out, maximum);
    ...
    public synchronized void retainEndpoints(Set<DiagnosticHostPolicy.Endpoint> retained) {
        ...
        policy.endpointExpiries()
                .forEach(
                        (endpoint, expiry) -> {
                            if (retained.contains(endpoint)) {
                                deadlines.put(endpoint, expiry);
                            }
                        });
        policy =
    ...
    public IceUdpMuxListener.Acceptance admit(IceUdpMuxListener.Request request) {
        return admit(request, null);
    }

    /** Takes ownership of a token already verified by the shared player/diagnostic admission path. */
    public synchronized IceUdpMuxListener.Acceptance admit(
            IceUdpMuxListener.Request request, VerifiedAdmission verified) {
        ...
        if (closed
                ...
                || sessions.size() >= 4
                || used.size() >= 16
                || request.localUfrag().length() < 8
                || !request.localUfrag().startsWith("NXS1")) {
            if (verified != null) {
                verified.identityVerifier().close();
            }
            rejected++;
            return null;
        }
        ...
        try {
            ...
            InetSocketAddress remote =
                    new InetSocketAddress(
                            EndpointAddress.parse(request.remoteAddress()), request.remotePort());
            int family = remote.getAddress() instanceof Inet6Address ? 6 : 4;
            if (claims.profile() != PROFILE
                    || family != claims.family()
                    || claims.expiresAt()
                            > policy.endpointExpiry(DiagnosticHostPolicy.Endpoint.from(claims))
                    || used.containsKey(claims.attemptIdHex())
                    || sessions.values().stream().anyMatch(s -> s.remote.equals(remote))) {
                throw invalid();
            }
            long remaining = claims.expiresAt() - now;
            if (remaining < 1 || remaining > MAX_ATTEMPT_MILLIS) {
                throw invalid();
            }
            Session session =
                    new Session(
                            ...
                            key,
                            remote,
                            nanos + remaining * 1_000_000L,
                            nanos + Math.min(remaining, MAX_HANDSHAKE_MILLIS) * 1_000_000L);
            sessions.put(claims.attemptIdHex(), session);
            used.put(claims.attemptIdHex(), claims.expiresAt());
            request.completion().whenComplete((peer, failure) -> settled(session, failure));
            return IceUdpMuxListener.Acceptance.builder(
                            remoteDescription(admission, remote), admission.credentials().icePwd())
                    .configuration(
                    ...
                    .expiresAt(Instant.ofEpochMilli(claims.expiresAt()))
                    .initialize(peer -> initialize(session, peer))
                    .build();
        } catch (RuntimeException | UnknownHostException invalid) {
            rejected++;
            return null;
        }
    }
    ...
    public synchronized String assist(AssistedJoin join, Runnable requireCurrent) {
        Objects.requireNonNull(requireCurrent);
        requireCurrent.run();
        long now = observe(), nanos = lastNanos;
        prune(now);
        if (!join.diagnostic()
                ...
                || sessions.size() >= 4
                || used.size() >= 16
                || !join.hostFingerprint().equalsIgnoreCase(identity.fingerprint())
                || !join.instanceId().equals(policy.context().hostId())
                || join.generation() != policy.context().generation()
                || !join.incarnation().equals(policy.context().incarnation())) {
            throw invalid();
        }
        ...
        try {
            Claims claims = admission.claims();
            var remote =
                    DiagnosticSdp.offer(utf8(join.offer()), claims, remoteUfrag)
                            .endpoint(claims.family());
            var scope = EndpointAddress.scope(remote.getAddress());
            boolean privateIpv4Host =
                    claims.family() == 4
                            && scope == EndpointAddress.Scope.PRIVATE
                            && join.offer()
                                    .lines()
                                    ...
                                    .split(" ")[7]
                                    .equals("host");
            if (claims.profile() != ASSISTED_PROFILE
                    || !claims.attemptIdHex().equals(join.id())
                    || claims.expiresAt() != join.expiresAt()
                    ...
                    || (scope != EndpointAddress.Scope.PUBLIC
                            && !privateIpv4Host
                            && !(listenerAddress.getAddress().isLoopbackAddress()
                                    && scope == EndpointAddress.Scope.LOOPBACK))) {
                throw invalid();
            }
            long remaining = claims.expiresAt() - now;
            if (remaining < 1 || remaining > MAX_ATTEMPT_MILLIS) {
            ...
            session.peer =
                    PeerConnection.createPeer(
                            PeerConnectionConfiguration.DEFAULT
                                    .withBindAddress(listenerAddress.getAddress())
                                    .withPortRangeBegin(listenerAddress.getPort())
                                    .withPortRangeEnd(listenerAddress.getPort())
                                    .withEnableIceUdpMux(true)
                                    .withIceServers(List.of())
                                    .withEnableIceTcp(false)
                                    .withDisableAutoNegotiation(true)
                                    .withMtu(1248)
                                    .withMaxMessageSize(MAX_MESSAGE_SIZE),
                            Runnable::run,
    ...
    public synchronized IceUdpMuxListener.Acceptance reuse(IceUdpMuxListener.Request request) {
        Session session =
                sessions.values().stream()
                        .filter(
                                s ->
                                        s.requireCurrent != null
                                                && s.admission
                                                        .credentials()
                                                        .localUfrag()
                                                        .equals(request.localUfrag()))
                        .findFirst()
                        .orElse(null);
        if (session == null
                || session.peer == null
                || session.closing
                || !authorized(session, observe())
                || !session.admission.remoteUfrag().equals(request.remoteUfrag())) {
            return null;
        }
        try {
            if (!assistedRemoteAllowed(
                    session,
                    new InetSocketAddress(
                            EndpointAddress.parse(request.remoteAddress()),
                            request.remotePort()))) {
                return null;
            }
        ...
        return IceUdpMuxListener.Acceptance.reuse(
                session.peer, Instant.ofEpochMilli(session.admission.claims().expiresAt()));
    }

    private boolean assistedRemoteAllowed(Session session, InetSocketAddress remote) {
        var scope = EndpointAddress.scope(remote.getAddress());
        return (remote.getAddress() instanceof Inet6Address ? 6 : 4)
                        == session.admission.claims().family()
                && (scope == EndpointAddress.Scope.PUBLIC
                        || listenerAddress.getAddress().isLoopbackAddress()
                                && scope == EndpointAddress.Scope.LOOPBACK);
    }
    ...
    public synchronized void requireAssistedCurrent(AssistedJoin join) {
        ...
        if (session == null
                || session.closing
                || !session.admission.credentials().localUfrag().equals(join.localUfrag())
                || !authorized(session, observe())) {
            throw invalid();
        }
    }
    ...
    public synchronized void cancelAssisted(AssistedJoin join) {
        ...
        if (session != null
                && session.admission.credentials().localUfrag().equals(join.localUfrag())) {
            stop(session, "assisted_answer_failed");
        }
    }

    /** Trusted same-mux gatherer capture, before the provider returns the answer. */
    public synchronized void assistedAnswerCandidate(
            AssistedJoin join, InetSocketAddress candidate) {
        requireAssistedCurrent(join);
        Session session = sessions.get(join.id());
        if (candidate.isUnresolved()
                || candidate.getPort() < 1
                || (candidate.getAddress() instanceof Inet6Address ? 6 : 4)
                        != session.admission.claims().family()) {
            throw invalid();
        }
    ...
    private synchronized void initialize(Session session, PeerConnection peer) {
        session.peer = peer;
        session.created = true; // Own it before anything that may reject initialization.
        if (!authorized(session, observe()) || session.wantsClose) {
            throw invalid();
        }
        String local = peer.localDescription();
        if (!local.contains("a=fingerprint:" + identity.fingerprint() + "\r\n")
                || !local.contains(
                        "a=ice-ufrag:" + session.admission.credentials().localUfrag() + "\r\n")) {
            throw invalid();
        }
    ...
    private synchronized void settled(Session session, Throwable failure) {
        ...
        if (failure != null) {
            ...
            session.reason = "native_acceptance_failed";
            session.wantsClose = true;
        }
        if (session.wantsClose || closed) {
            stop(session, session.reason);
        }
    }
    ...
    private boolean authorized(Session session, long now) {
        ...
        try {
            if (session.requireCurrent != null) {
                session.requireCurrent.run();
            }
        ...
        return !closed
                && closeFailure == null
                && !clockFailed
                && session.admission.context().equals(policy.context())
                && now < claims.expiresAt()
                && lastNanos - session.deadlineNanos < 0
                && keyEquals(session.key, policy.key(session.key.keyId()))
                && now >= session.key.notBefore()
                && now < session.key.retireAt()
                && policy.endpoints().contains(DiagnosticHostPolicy.Endpoint.from(claims));
    }

    private static boolean keyEquals(Key first, Key second) {
    ...
    public synchronized void tick() {
        ...
        prune(now);
        for (Session session : new ArrayList<>(sessions.values())) {
            if (session.closing || session.wantsClose) {
                continue;
            }
            if (!authorized(session, now)) {
                stop(session, "expired_or_withdrawn");
                continue;
            }
            if (session.channels.protocolFailed()) {
                stop(session, "invalid_diagnostic_protocol");
                continue;
            }
            if (session.channels.failed()) {
                stop(session, session.complete ? "pong_sent" : "transport_failed");
                continue;
            }
            if (!session.complete && lastNanos - session.handshakeDeadlineNanos >= 0) {
                stop(session, "handshake_timeout");
                continue;
            }
            try {
                ...
                if (session.exchange == null && ready) {
                    ...
                    session.exchange =
                            new DiagnosticExchange(
                                    session.admission.claims().attemptIdHex(),
                                    true,
                                    (index, bytes) -> {
                                        if (!authorized(session, observe()) || session.closing) {
                                            throw invalid();
                                        }
                ...
                // The client closes as soon as it verifies the pong. Bound hosts whose client
                // disappears.
                if (session.complete && lastNanos - session.completeNanos >= 1_000_000_000L) {
                    stop(session, "pong_sent");
                }
            } catch (RuntimeException invalid) {
                stop(session, "invalid_" + session.phase);
            }
        }
    }
    ...
    private void capture(Session session) {
        ...
        try {
            InetSocketAddress local =
                    new InetSocketAddress(
                            EndpointAddress.parse(pair.local().getHostString()),
                            pair.local().getPort());
            InetSocketAddress remote =
                    new InetSocketAddress(
                            EndpointAddress.parse(pair.remote().getHostString()),
                            pair.remote().getPort());
            boolean boundLocal =
                    local.getPort() == listenerAddress.getPort()
                            && (listenerAddress.getAddress().isAnyLocalAddress()
                                    || listenerAddress.getAddress().equals(local.getAddress()));
            boolean remoteAllowed =
                    session.requireCurrent == null
                            ? remote.equals(session.remote)
                            : assistedRemoteAllowed(session, remote);
            if (!remoteAllowed
                    || !(boundLocal
                            || session.requireCurrent != null
                                    && local.equals(session.gatheredLocal))
                    || (local.getAddress() instanceof Inet6Address ? 6 : 4)
                            != session.admission.claims().family()
                    || !"udp".equals(pair.localTransport())
                    || !"udp".equals(pair.remoteTransport())) {
                throw invalid();
            }
            session.selectedLocal = local;
            session.selectedRemote = remote;
    ...
    private void stop(Session session, String reason) {
        ...
        if (!reason.equals("pong_sent")) {
            session.complete = false;
        }
        if (!session.settled) {
            return;
        } // The listener owns any in-progress prepare until completion settles.
        ...
        try {
            session.peer
                    .closeAsync()
                    .whenComplete((ignored, failure) -> finished(session, failure));
        } catch (RuntimeException failure) {
            finished(session, failure);
        }
    }
    ...
    private synchronized void finished(Session session, Throwable failure) {
        ...
        if (!session.created) {
            used.remove(session.admission.claims().attemptIdHex());
        }
        long now = observe();
        boolean success =
                failure == null
                        && session.complete
                        && !session.channels.protocolFailed()
                        && authorized(session, now);
        if (session.created) {
            ...
            Result result =
                    new Result(
                            ...
                            claims.expiresAt(),
                            DiagnosticHostPolicy.Endpoint.from(claims),
                            success,
                            failure != null
                            ...
                            session.selectedRemote,
                            exchange == null ? 0 : exchange.sentFrames(),
                            exchange == null ? 0 : exchange.sentBytes(),
                            session.channels.receivedFrames(),
                            session.channels.receivedBytes(),
                            now,
                            failure == null);
    ...
    private void prune(long now) {
        used.entrySet()
                .removeIf(
                        entry -> entry.getValue() <= now && !sessions.containsKey(entry.getKey()));
    }

    private long observe() {
        long wall = clock.wallMillis().getAsLong(), nanos = clock.nanoTime().getAsLong();
        if (clockFailed || wall < 0 || wall > SAFE || anchored && nanos - lastNanos < 0) {
            clockFailed = true;
            return SAFE;
        }
        if (!anchored) {
        ...
        if (elapsed < 0 || anchorMillis > SAFE - elapsed / 1_000_000L) {
            clockFailed = true;
            return SAFE;
        }
    ...
    private static String remoteDescription(Admission admission, InetSocketAddress remote) {
        String fingerprint =
                HexFormat.ofDelimiter(":")
                        .withUpperCase()
                        .formatHex(unhex(admission.claims().clientFingerprintHex(), 32));
        return VerifiedAdmission.remoteDescription(
                        admission.remoteUfrag(),
                        admission.claims().clientIcePwd(),
                        "sha-256 " + fingerprint,
                        SCTP_PORT,
                        MAX_MESSAGE_SIZE)
