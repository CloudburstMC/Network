...
public final class NativeDiagnosticProbeAttempt implements AutoCloseable {
    ...
    public record Job(
            ...
            String hostFingerprintHex,
            long expiresAt) {
        public Job {
            Objects.requireNonNull(context);
            Objects.requireNonNull(target);
            unhex(attemptIdHex, 16);
            unhex(hostFingerprintHex, 32);
    ...
    public record Request(Context context, Claims claims, String ufrag, byte[] offer) {
        public Request {
            ...
            DiagnosticAdmissionCodec.ufrag(ufrag);
            if (offer.length < 1 || offer.length > DiagnosticSdp.MAX_BYTES) {
                throw invalid();
            }
            offer = offer.clone();
        }
        ...
        public String toString() {
            return "NativeDiagnosticProbeRequest[redacted]";
        }
    }
    ...
    public interface Signaling {
        CompletionStage<String> exchange(Request request);
    }
    ...
    public NativeDiagnosticProbeAttempt(
            Job job, InetSocketAddress bind, BooleanSupplier authorized) {
    ...
    NativeDiagnosticProbeAttempt(
        ...
        if (stunServer != null
                && (!job.target.assisted()
                        || stunServer.isUnresolved()
                        || stunServer.getPort() < 1
                        || family(stunServer.getAddress()) != job.target.family()
                        || !(EndpointAddress.scope(stunServer.getAddress())
                                        == EndpointAddress.Scope.PUBLIC
                                || loopbackTest
                                        && EndpointAddress.scope(stunServer.getAddress())
                                                == EndpointAddress.Scope.LOOPBACK))) {
            throw invalid();
        }
                    ...
                    job.target.assisted()
                            ...
                            : new InetSocketAddress(
                                    InetAddress.getByAddress(
                                            job.target.family() == 4
                                                    ? Arrays.copyOfRange(address, 12, 16)
                                                    : address),
                                    job.target.port());
            if (target != null) {
                EndpointAddress.Scope scope = EndpointAddress.scope(target.getAddress());
                if (scope != EndpointAddress.Scope.PUBLIC
                        && !(loopbackTest && scope == EndpointAddress.Scope.LOOPBACK)) {
                    throw invalid();
                }
            }
            if (bind.isUnresolved()
                    || bind.getPort() < 1
                    || bind.getAddress().isAnyLocalAddress()
                    || bind.getAddress().isMulticastAddress()
                    || family(bind.getAddress()) != job.target.family()
                    || bind.getAddress() instanceof Inet6Address v6 && v6.getScopeId() != 0) {
                throw invalid();
            }
            this.bind =
                    new InetSocketAddress(
                            InetAddress.getByAddress(bind.getAddress().getAddress()),
                            bind.getPort());
    ...
    public Result run(Signaling signaling) {
        ...
        if (!started.compareAndSet(false, true)) {
            throw new IllegalStateException("Diagnostic attempt already used");
        }
        Reason reason = Reason.GATHERING;
        DiagnosticExchange exchange = null;
        InetSocketAddress selectedLocal = null, selectedRemote = null, attemptedRemote = null;
        boolean answerVerified = false,
        ...
        boolean discoveryClean = true;
        try {
            anchorWall = clock.wallMillis().getAsLong();
            anchorNanos = clock.nanoTime().getAsLong();
            currentNanos = anchorNanos;
            previousNanos = anchorNanos;
            currentWall = anchorWall;
            integer(anchorWall, 0, SAFE);
            long remaining = job.expiresAt - anchorWall;
            if (remaining < 1 || remaining > MAX_ATTEMPT_MILLIS) {
                throw new Failed(Reason.EXPIRED);
            }
            deadlineNanos = anchorNanos + remaining * 1_000_000L;
            handshakeDeadlineNanos =
                    anchorNanos + Math.min(remaining, MAX_HANDSHAKE_MILLIS) * 1_000_000L;
            check();
            var configuration =
                    PeerConnectionConfiguration.DEFAULT
                            .withIceServers(List.of())
                            .withEnableIceTcp(false)
                            .withBindAddress(bind.getAddress())
                            .withPortRangeBegin(bind.getPort())
                            .withPortRangeEnd(bind.getPort())
            ...
            peer.set(nativePeer);
            check();
            nativePeer.onGatheringStateChange.register(
                    (p, state) -> {
            ...
            checkHandshake();
            nativePeer.setLocalDescription("offer", ufrag, password);
            await(gathered::get, true);
            String offerText = nativePeer.localDescription();
            if (offerText.length() > DiagnosticSdp.MAX_BYTES) {
            ...
            if (job.target.assisted()) {
                ...
                if (family(candidate.getAddress()) != job.target.family()
                ...
                String replacement =
                        "1 1 UDP 2130706431 "
                                + candidate.getAddress().getHostAddress()
                                + " "
                                + candidate.getPort()
                                + " typ "
                                + (monitor == null ? "host" : "srflx");
                offerText =
                        offerText.replace("a=candidate:" + original, "a=candidate:" + replacement);
            }
            byte[] offer = utf8(offerText);
            offerHash = hex(digest(offer));
            ...
            var claims =
                    new Claims(
                            ...
                            job.attemptIdHex,
                            offerHash,
                            job.target.candidateRevision(),
                            job.target.family(),
                            job.target.addressHex(),
                            job.target.port(),
                            job.target.assisted() ? ASSISTED_PROFILE : PROFILE);
            DiagnosticSdp offered = DiagnosticSdp.offer(offer, claims, ufrag);
            if (!job.target.assisted() && !offered.endpoint(job.target.family()).equals(bind)) {
                throw new Failed(Reason.GATHERING);
            }
            checkHandshake();
            reason = Reason.SIGNALING;
            pending =
                    Objects.requireNonNull(
                                    signaling.exchange(
                                            new Request(job.context, claims, ufrag, offer)))
                            .toCompletableFuture();
            await(pending::isDone, true);
            String sdp = pending.join();
            pending = null;
            check();
            reason = Reason.ANSWER;
            if (sdp == null || sdp.length() > DiagnosticSdp.MAX_BYTES) {
            ...
            CandidatePair establishedPair =
                    selectedPair(nativePeer); // No PING is sent to an invalid selected destination.
            selectedLocal =
                    numeric(
                            establishedPair.local().getHostString(),
                            establishedPair.local().getPort());
            selectedRemote =
                    numeric(
                            establishedPair.remote().getHostString(),
                            establishedPair.remote().getPort());
            ...
            exchange =
                    new DiagnosticExchange(
                            ...
                            (channel, bytes) -> {
                                check();
                                channels.send(channel, bytes);
                            });
            exchange.start();
            while (!exchange.complete()) {
                check();
                if (channels.protocolFailed()) {
                    throw new Failed(Reason.PROTOCOL);
                }
                byte[] incoming = channels.poll();
                if (incoming != null) {
                    exchange.receive(0, incoming);
                }
        ...
        } catch (HandshakeTimeout timeout) {
            reason = answerVerified && !transportEstablished ? Reason.TRANSPORT : Reason.EXPIRED;
            ...
            /* Reason identifies the failing bounded stage; never include secret payloads. */
        } finally {
            if (pending != null) {
            ...
            if (value == null) {
                ...
                try {
                    closePeer(value).toCompletableFuture().get(5, TimeUnit.SECONDS);
                    cleanup = true;
                    peer.compareAndSet(value, null);
        ...
        if (complete && cleanup) {
            try {
                ...
                complete = false;
                reason = withdrawn instanceof Failed failure ? failure.reason : Reason.WITHDRAWN;
            }
        }
        boolean success = complete && cleanup && !channels.protocolFailed();
        if (!success && reason == Reason.COMPLETE) {
            reason = Reason.PROTOCOL;
        }
        return new Result(
                job,
                ...
                exchange != null && exchange.complete(),
                cleanup,
                offerHash,
                fingerprint,
                selectedLocal,
                ...
                channels.receivedFrames(),
                channels.receivedBytes(),
                currentWall);
    }
    ...
    private CandidatePair selectedPair(PeerConnection nativePeer) {
        ...
        InetSocketAddress selectedLocal =
                numeric(pair.local().getHostString(), pair.local().getPort());
        InetSocketAddress selectedRemote =
                numeric(pair.remote().getHostString(), pair.remote().getPort());
        ...
        boolean assistedLocal =
                job.target.assisted()
                        && (selectedLocal.equals(gatheredLocal)
                                || "prflx".equals(pair.localType())
                                        && (localScope == EndpointAddress.Scope.PUBLIC
                                                || loopbackTest
                                                        && localScope
                                                                == EndpointAddress.Scope.LOOPBACK));
        if (!(selectedLocal.equals(bind) || assistedLocal)
                || (target != null
                        ? !selectedRemote.equals(target)
                        : EndpointAddress.scope(selectedRemote.getAddress())
                                        != EndpointAddress.Scope.PUBLIC
                                && !(loopbackTest
                                        && EndpointAddress.scope(selectedRemote.getAddress())
                                                == EndpointAddress.Scope.LOOPBACK))
                || family(selectedLocal.getAddress()) != job.target.family()
                || family(selectedRemote.getAddress()) != job.target.family()
                || !"udp".equals(pair.localTransport())
                || !"udp".equals(pair.remoteTransport())) {
            throw new Failed(Reason.SELECTED_PATH);
        }
        return pair;
    }
    ...
    private void checkHandshake() {
        check();
        if (currentNanos - handshakeDeadlineNanos >= 0) {
            throw new HandshakeTimeout();
        }
    }
    ...
    private void pause(boolean handshake) throws InterruptedException {
        ...
        if (channels.failed()) {
            throw new Failed(Reason.TRANSPORT);
        }
        if (handshake && currentNanos - handshakeDeadlineNanos >= 0) {
            throw new HandshakeTimeout();
        }
        Thread.sleep(5);
    }
    ...
    private void check() {
        if (cancelled.get() || Thread.currentThread().isInterrupted()) {
            throw new Failed(Reason.CANCELLED);
        }
        long nanos = clock.nanoTime().getAsLong(), wall = clock.wallMillis().getAsLong();
        integer(wall, 0, SAFE);
        if (nanos - previousNanos < 0) {
            throw new Failed(Reason.EXPIRED);
        }
        currentNanos = nanos;
        previousNanos = nanos;
        long progressed = anchorWall + (nanos - anchorNanos) / 1_000_000L;
        currentWall = Math.max(wall, progressed);
        anchorWall = currentWall;
        anchorNanos = nanos;
        if (nanos - deadlineNanos >= 0 || currentWall >= job.expiresAt) {
            throw new Failed(Reason.EXPIRED);
        }
        if (!authorized.getAsBoolean()) {
            throw new Failed(Reason.WITHDRAWN);
        }
    }

    private static InetSocketAddress numeric(String value, int port) {
        try {
            return new InetSocketAddress(EndpointAddress.parse(value), port);
        } catch (UnknownHostException e) {
            throw invalid();
        }
    }

    private static int family(InetAddress address) {
        return address instanceof Inet6Address ? 6 : 4;
    }
    ...
    private static String field(String text, String prefix) {
        List<String> values = text.lines().filter(line -> line.startsWith(prefix)).toList();
        if (values.size() != 1) {
            throw invalid();
        }
    ...
    private CompletionStage<Void> closePeer(PeerConnection value) {
        ...
        try {
            closing = value.closeAsync();
        } catch (RuntimeException failure) {
            termination.completeExceptionally(failure);
            return CompletableFuture.<Void>failedFuture(failure).minimalCompletionStage();
        }
