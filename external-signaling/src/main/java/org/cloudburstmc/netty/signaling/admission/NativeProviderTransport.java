...
public final class NativeProviderTransport implements ProviderTransport {
    ...
    private final long diagnosticAnchorNanos = System.nanoTime(),
            diagnosticAnchorMillis = System.currentTimeMillis();
    ...
    private long diagnosticNow() {
        ...
        if (elapsed < 0) {
            throw new IllegalStateException("Diagnostic monotonic clock rollback");
        }
        long now =
                Math.max(
                        System.currentTimeMillis(),
                        Math.addExact(diagnosticAnchorMillis, elapsed / 1_000_000));
        return diagnosticTime.accumulateAndGet(now, Math::max);
    }

    private NativeProviderTransport(
            NativeAdmissionServerChannel channel,
            StatelessAdmissionValidator validator,
            String incarnation,
            Supplier<List<InetSocketAddress>> advertisedAddresses,
    ...
    public static CompletionStage<NativeProviderTransport> openMaintained(
            ServerBootstrap bootstrap,
            EndpointSelection selection,
            Map<EndpointSelection.Family, InetSocketAddress> numericStunServers,
            Path certificate,
            Path privateKey,
            AdmissionGate.Limits limits) {
        return openMaintained(
                bootstrap, selection, numericStunServers, certificate, privateKey, limits, false);
    }
    ...
    public static CompletionStage<NativeProviderTransport> openMaintained(
        ...
        var direct =
                NativeCandidateSnapshot.hosts(
                        selection.candidates().stream()
                                .map(EndpointSelection.Candidate::endpoint)
                                .toList());
        return open(bootstrap, selection.bind(), null, certificate, privateKey, limits, direct)
                .thenCompose(
                        transport -> {
                            var controller =
                                    selection.configured() || assistedJoins || !stunWarming
                                            ? CompletableFuture
                                                    .<org.cloudburstmc.netty.signaling.provider
                                                                    .connectivity
                                                                    .EndpointConnectivityController>
                                                            completedFuture(null)
                                            : transport.channel.enableConnectivity(
                                                    selection, servers, Duration.ofMinutes(5));
                            return controller
                                    .thenApply(
                                            value -> {
                                                synchronized (transport) {
                                                    if (transport.closed
                                                            || !transport.channel.isActive()) {
                                                        throw new IllegalStateException(
                                                                "Native listener closed");
                                                    }
                                                    transport.candidatePublisher =
                                                            new MaintainedCandidatePublisher(
                                                                    selection,
                                                                    value,
                                                                    assistedJoins);
                                                    transport.candidatePublisher
                                                            .configureStunServers(servers);
                                                    transport.refreshMaintained();
                                                    transport.candidateTask =
                                                            transport
                                                                    .channel
                                                                    .eventLoop()
    ...
    private static CompletionStage<NativeProviderTransport> open(
            ServerBootstrap bootstrap,
    ...
    private boolean replaceCandidateMaterial(
            NativeCandidateSnapshot next, boolean identityChanged) {
        if (!fixedCandidates || closed || draining || !channel.isActive()) {
            throw new IllegalStateException("Native listener unavailable");
        }
        if (candidateSnapshot.equals(next) && !identityChanged) {
            return false;
        }
        if (candidateGeneration >= 9007199254740991L) {
            throw new IllegalStateException("Candidate revision exhausted");
        }
    ...
    private NativeCandidateSnapshot currentCandidates() {
        ...
        ordinaryCandidateOrder =
                advertised.stream()
                        .map(
                                endpoint ->
                                        new NativeCandidateSnapshot.Candidate(
                                                endpoint, NativeCandidateSnapshot.Type.HOST))
                        .toList();
        if (candidateSnapshot == null) {
            candidateSnapshot = next;
        } else if (!candidateSnapshot.equals(next)) {
            if (diagnosticGate != null) {
                diagnosticGate.retainEndpoints(Set.of());
            }
            if (candidateGeneration >= 9007199254740991L) {
                throw new IllegalStateException("Candidate revision exhausted");
    ...
    private synchronized void refreshMaintained() {
        if (closed || draining || !channel.isActive() || candidatePublisher == null) {
            return;
        }
        try {
            var next = candidatePublisher.refresh();
            boolean changed =
                    replaceCandidateMaterial(
                            next.probeCandidates(),
                            next.mappingRevision() != maintainedMappingRevision);
            maintainedMappingRevision = next.mappingRevision();
            if (candidatePublication == null
                    || changed
                    || !candidatePublication.candidates().equals(next.candidates())) {
    ...
    public CompletionStage<Void> configureStunServers(List<StunServer> servers) {
        if (servers.size() > 2) {
            return CompletableFuture.failedFuture(
                    new IllegalArgumentException("At most two provider STUN servers"));
        }
        synchronized (this) {
            if (closed || candidatePublisher == null || !candidatePublisher.needsStunServers()) {
                return CompletableFuture.completedFuture(null);
            }
        }
        // ProviderClient invokes configuration from its background discovery task, never the native
        // loop.
        var numeric =
                new java.util.EnumMap<EndpointSelection.Family, InetSocketAddress>(
                        EndpointSelection.Family.class);
        for (var server : servers) {
            try {
                for (var address : java.net.InetAddress.getAllByName(server.host())) {
                    if (!address.isAnyLocalAddress() && !address.isMulticastAddress()) {
                        numeric.putIfAbsent(
                                EndpointSelection.Family.of(address),
                                new InetSocketAddress(address, server.port()));
                    }
                }
            } catch (java.net.UnknownHostException unavailable) {
                /* Unavailable discovery remains unknown. */
            }
        }
            ...
            channel.eventLoop()
                    .execute(
                            () -> {
                                try {
                                    synchronized (this) {
                                        if (closed || draining || candidatePublisher == null) {
                                            throw new IllegalStateException(
                                                    "Native listener unavailable");
                                        }
                                        candidatePublisher.configureStunServers(numeric);
                                        refreshMaintained();
                                    }
    ...
    @Override
    public CompletionStage<Void> reportConnectivityChecks(
            long revision, List<ConnectivityCheck> checks) {
        Objects.requireNonNull(checks);
        if (checks.size() > 18) {
            throw new IllegalArgumentException("At most eighteen connectivity checks");
        }
        ...
        synchronized (this) {
            if (closed
                    ...
                    || candidateGeneration != revision) {
                return CompletableFuture.completedFuture(null);
            }
            if (connectivityFeedbackPending) {
                return CompletableFuture.failedFuture(
                        new IllegalStateException("Connectivity feedback already pending"));
            }
                    ...
                    .execute(
                            () -> {
                                try {
                                    ...
                                    synchronized (this) {
                                        connectivityFeedbackPending = false;
                                    }
                                    result.complete(null);
                                } catch (RuntimeException failure) {
                                    synchronized (this) {
    ...
    public CompletionStage<Void> configureDiagnostics(DiagnosticHostPolicy policy) {
        ...
        if (remaining <= 0) {
            return CompletableFuture.failedFuture(
                    new IllegalArgumentException("Expired diagnostic configuration"));
        }
        return configureDiagnostics(policy, started + TimeUnit.MILLISECONDS.toNanos(remaining));
    }

    @Override
    public CompletionStage<Void> configureDiagnostics(
            DiagnosticHostPolicy policy, long deadlineNanos) {
        Objects.requireNonNull(policy);
        long remaining = deadlineNanos - System.nanoTime();
        if (remaining <= 0 || remaining > TimeUnit.MINUTES.toNanos(5)) {
            return CompletableFuture.failedFuture(
                    new IllegalArgumentException("Diagnostic deadline"));
        }
        var result = new CompletableFuture<Void>();
        final long mutation;
        synchronized (this) {
            if (closed || draining || !channel.isActive()) {
                return CompletableFuture.failedFuture(
                        new IllegalStateException("Native listener unavailable"));
            }
            if (diagnosticConfiguring) {
                return CompletableFuture.failedFuture(
                        new IllegalStateException("Diagnostic configuration already in flight"));
            }
            diagnosticConfiguring = true;
            diagnosticMutation++;
                            ...
                            () -> {
                                ...
                                synchronized (NativeProviderTransport.this) {
                                    try {
                                        ...
                                        if (diagnosticMutation != mutation
                                                || closed
                                                || draining
                                                || !channel.isActive()) {
                                            throw new IllegalStateException(
                                                    "Diagnostic configuration cancelled");
                                        }
                                        if (now >= policy.expiresAt()
                                                || policy.endpointExpiries().values().stream()
                                                        .anyMatch(expiry -> now >= expiry)) {
                                            throw new IllegalArgumentException(
                                                    "Expired diagnostic configuration");
                                        }
                                        NativeCandidateSnapshot current =
                                                candidatePublication == null
                                                        ? currentCandidates()
                                                        : candidatePublication.probeCandidates();
                                        if (!incarnation.equals(policy.context().incarnation())
                                                || policy.endpoints().stream()
                                                        .anyMatch(
                                                                endpoint ->
                                                                        endpoint.candidateRevision()
                                                                                        != candidateGeneration
                                                                                || (endpoint
                                                                                                .assisted()
                                                                                        ? !eligibleAssistedFamilies(
                                                                                                        current
                                                                                                                .candidates())
                                                                                                .contains(
                                                                                                        endpoint
                                                                                        ...
                                                                                        : !containsEndpoint(
                                                                                                ...
                                                                                                endpoint)))) {
                                            throw new IllegalArgumentException(
                                                    "Diagnostic listener or endpoint mismatch");
                                        }
                                        if (candidatePublication != null) {
                                                ...
                                                Math.min(
                                                        ...
                                                        System.currentTimeMillis()
                                                                + TimeUnit.NANOSECONDS.toMillis(
                                                                        nanosLeft));
                                        var endpointExpiries =
                                                new HashMap<DiagnosticHostPolicy.Endpoint, Long>();
                                        policy.endpointExpiries()
                                                .forEach(
                                                        (endpoint, limit) ->
                                                                endpointExpiries.put(
                                                                        endpoint,
                                                                        Math.min(limit, expiry)));
                                        var bounded =
                                                new DiagnosticHostPolicy(
                                                        ...
                                                        endpointExpiries);
                                        diagnosticGate =
                                                channel.installDiagnostics(bounded, diagnosticGate);
                                        diagnosticPolicy = bounded;
                                    } catch (Throwable problem) {
                                        failure = problem;
                                    } finally {
                                        diagnosticConfiguring = false;
                                    }
                                }
    ...
    private static boolean containsEndpoint(
            NativeCandidateSnapshot snapshot, DiagnosticHostPolicy.Endpoint endpoint) {
        return snapshot.candidates().stream()
                .anyMatch(candidate -> sameEndpoint(candidate, endpoint));
    }

    private static boolean sameEndpoint(
            NativeCandidateSnapshot.Candidate candidate, DiagnosticHostPolicy.Endpoint endpoint) {
        var address = candidate.endpoint().getAddress();
        int family = address instanceof java.net.Inet6Address ? 6 : 4;
        return family == endpoint.family()
                && candidate.endpoint().getPort() == endpoint.port()
                && DiagnosticAdmissionCodec.address(family, address.getHostAddress())
                        .equals(endpoint.addressHex());
    }

    /** Withdraw new and active diagnostics, retaining the local anti-replay history. Player admission is untouched. */
    @Override
    public synchronized CompletionStage<Void> disableDiagnostics() {
        ++diagnosticMutation;
        if (diagnosticGate != null) {
            diagnosticGate.retainEndpoints(Set.of());
        }
    ...
    public synchronized java.util.OptionalLong diagnosticDroppedResultCount() {
        return diagnosticGate == null
                ? java.util.OptionalLong.empty()
                : java.util.OptionalLong.of(diagnosticGate.stats().droppedResults());
    }
    ...
    public CompletionStage<String> assistedJoin(
            org.cloudburstmc.netty.signaling.control.AssistedJoin join, Runnable requireCurrent) {
        Runnable guard =
                () -> {
                    requireCurrent.run();
                    if (closed
                            || draining
                            || !channel.isServing()
                            || !incarnation.equals(join.incarnation())
                            || !validator.keyIds().contains(join.keyId())
                            || epochs.stream()
                                    .noneMatch(
                                            e ->
                                                    e.id().equals(join.keyId())
                                                            && e.notBefore()
                                                                    <= System.currentTimeMillis()
                                                            && e.retireAfter()
                                                                    > System.currentTimeMillis()
                                                            && e.retireAfter()
                                                                    >= join.expiresAt())) {
                        throw new IllegalStateException("Assisted native identity unavailable");
                    }
                };
        guard.run();
        ...
        synchronized (this) {
            stunServers =
                    candidatePublisher == null
                            ? Map.of()
                            : candidatePublisher.assistedStunServers();
            if (candidateSnapshot != null) {
                for (var candidate : candidateSnapshot.candidates()) {
                    var endpoint = candidate.endpoint();
                    if (candidate.type() == NativeCandidateSnapshot.Type.HOST
                            && EndpointAddress.scope(endpoint.getAddress())
                                    == EndpointAddress.Scope.PUBLIC) {
                        publicCandidates.putIfAbsent(
                                endpoint.getAddress() instanceof java.net.Inet4Address ? 4 : 6,
                                endpoint);
                    }
                }
            }
        ...
        return join.diagnostic()
                ? channel.assistDiagnostic(join, guard, stunServers, selected)
    ...
    public synchronized CompletionStage<HostProfileSnapshot> captureHostProfile() {
        ...
        JsonArray candidates =
                encodeCandidates(
                        candidatePublication == null
                                ? endpoints
                                : candidatePublication.candidates().candidates());
                ...
                new HostProfileSnapshot(
                        ...
                        () -> {
                            ...
                            if (candidateGeneration != capturedGeneration
                                    || publicationVersion != capturedPublication) {
    ...
    private Set<Integer> eligibleAssistedFamilies(
            List<NativeCandidateSnapshot.Candidate> endpoints) {
        if (candidatePublication != null) {
            return candidatePublication.assistedFamilies();
        }
    ...
    public List<JsonObject> pollEvents(int maximum) {
        return channel.pollEvents(maximum).stream()
                .map(
                        event -> {
                            ...
                            result.addProperty(
                                    "occurredAt",
                                    Instant.ofEpochMilli(event.occurredAt()).toString());
                            if (event.remoteEndpoint() != null) {
                                result.addProperty(
                                        "remoteAddress",
                                        event.remoteEndpoint().getAddress().getHostAddress());
    ...
    public synchronized CompletionStage<Void> drain() {
        ...
        if (candidateTask != null) {
            candidateTask.cancel(false);
        }
        if (candidatePublisher != null) {
            candidatePublisher.close();
        }
