...
public final class NativeCandidateSnapshot {
    ...
    public record Candidate(InetSocketAddress endpoint, Type type) {
        public Candidate {
            ...
            if (endpoint == null
                    ...
                    || EndpointAddress.scope(endpoint.getAddress())
                            == EndpointAddress.Scope.UNUSABLE) {
                throw new IllegalArgumentException("Concrete numeric UDP endpoint required");
            }
            try {
                endpoint =
                        new InetSocketAddress(
                                InetAddress.getByAddress(endpoint.getAddress().getAddress()),
                                endpoint.getPort());
        ...
        public EndpointSelection.Family family() {
            return EndpointSelection.Family.of(endpoint.getAddress());
        }
    }
    ...
    public NativeCandidateSnapshot(List<Candidate> candidates) {
        ...
        if (candidates.size() > 32) {
            throw new IllegalArgumentException("At most 32 endpoint candidates");
        }
        this.candidates =
                candidates.stream()
                        ...
                        .distinct()
                        .sorted(
                                Comparator.comparing(Candidate::family)
                                        .thenComparing(
                                                c -> c.endpoint().getAddress().getHostAddress())
                                        .thenComparingInt(c -> c.endpoint().getPort())
                                        .thenComparing(Candidate::type))
                        .toList();
        StringBuilder material = new StringBuilder("native-candidates-v1\n");
        for (Candidate candidate : this.candidates) {
            material.append(candidate.family())
                    .append(' ')
                    .append(candidate.endpoint().getAddress().getHostAddress())
                    .append(' ')
                    .append(candidate.endpoint().getPort())
                    .append(' ')
                    .append(candidate.type().wire())
                    .append('\n');
        }
        try {
            materialRevision =
    ...
    public static NativeCandidateSnapshot hosts(List<InetSocketAddress> endpoints) {
        ...
        return new NativeCandidateSnapshot(
                endpoints.stream().map(endpoint -> new Candidate(endpoint, Type.HOST)).toList());
    }
    ...
    public boolean equals(Object value) {
        return value instanceof NativeCandidateSnapshot other
