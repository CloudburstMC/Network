...
public final class EndpointConnectivityController implements AutoCloseable {
    ...
    public record Sample(
        ...
        public Sample {
            ...
            Objects.requireNonNull(lastSuccessAge);
            EndpointSelection.requireEndpoint(server);
            if (mapped != null) {
                EndpointSelection.requireEndpoint(mapped);
            }
    ...
    public interface MonitorFactory {
        Monitor open(InetSocketAddress numericServer);
    }
    ...
    public record Observation(
            ...
            long freshUntilNanos) {
        /** Uses the same monotonic clock as this controller; freshness is never a reachability verdict. */
        public boolean freshAt(long nowNanos) {
            return lastSuccessAgeMillis >= 0 && nowNanos - freshUntilNanos < 0;
        }
    }

    public record FamilySnapshot(
            State state,
            List<EndpointSelection.Candidate> directCandidates,
            Optional<Observation> observation,
            Optional<InetSocketAddress> freshStunEndpoint) {
        public FamilySnapshot {
            directCandidates = List.copyOf(directCandidates);
        }
    }
    ...
    private static final class Lane {
        ...
        Lane(List<EndpointSelection.Candidate> direct, InetSocketAddress server) {
            this.direct = direct;
            this.server = server;
        }
    }
    ...
    public EndpointConnectivityController(
            EndpointSelection selection,
            Map<Family, InetSocketAddress> numericServers,
    ...
    EndpointConnectivityController(
        ...
        if (maxObservationAge.isNegative()
                || maxObservationAge.isZero()
                || maxObservationAge.compareTo(Duration.ofMinutes(5)) > 0) {
            throw new IllegalArgumentException(
                    "STUN observation age must be positive and at most five minutes");
        }
    ...
    public synchronized Snapshot snapshot() {
        ...
        for (Family family : Family.values()) {
            result.put(family, sample(family, lanes.get(family)));
        }
                ...
                closed
                        ...
                        : selection.configured()
                                ? (lane.direct.isEmpty()
                                        ...
                                        : State.CONFIGURED)
                                : !selection.socketFamilies().contains(family)
                                        ...
                                        : lane.failed
                                                ...
                                                : !lane.direct.isEmpty()
                                                        ? State.PUBLIC_DIRECT
                                                        : lane.server == null
                                                                ? State.STUN_NOT_CONFIGURED
                                                                : null;
        if (inactive != null) {
            return view(lane, inactive, null);
        }
        try {
            ...
            long sampledAt =
                    nanoTime.getAsLong(); // Conservative deadline: sampled before the native read.
            Optional<Sample> value = lane.monitor.read();
            if (value.isEmpty()) {
                setFresh(lane, null);
                return view(lane, State.STUN_PENDING, null);
            }
            Sample sample = value.get();
            if (!sample.server().equals(lane.server)
                    || sample.mapped() != null
                            && Family.of(sample.mapped().getAddress()) != family) {
                throw new IllegalStateException("STUN source/family mismatch");
            }
            long age =
                    sample.lastSuccessAge()
                            .map(EndpointConnectivityController::saturatedNanos)
                            .orElse(Long.MAX_VALUE);
            boolean observed =
                    sample.lastSuccessAge().isPresent()
                            && sample.mapped() != null
                            && sample.successfulResponses() > 0
                            && sample.mappingRevision() > 0;
            ...
            boolean eligible =
                    observed
                            && EndpointAddress.scope(sample.mapped().getAddress())
            ...
            Observation observation =
                    new Observation(
                            ...
                            sample.state(),
                            lane.epoch,
                            sample.mappingRevision(),
                            sample.successfulResponses(),
                            sample.failedTransactions(),
                            observed ? age / 1_000_000 : -1,
                            sampledAt + remaining);
            setFresh(lane, fresh && eligible ? sample.mapped() : null);
            return view(
                    lane,
                    !observed
                            ? (sample.state() == TransactionState.FAILED
                                    ? State.STUN_FAILED
                                    : State.STUN_PENDING)
                            : !fresh
                                    ? State.STUN_STALE
                                    : !eligible ? State.STUN_INELIGIBLE : State.STUN_FRESH,
                    observation);
    ...
    private FamilySnapshot view(Lane lane, State state, Observation observation) {
        return new FamilySnapshot(
                state,
                closed ? List.of() : lane.direct,
                Optional.ofNullable(observation),
                Optional.ofNullable(lane.fresh));
    }
    ...
    private static long saturatedNanos(Duration duration) {
        ...
        } catch (ArithmeticException tooOld) {
    ...
    private static void checkServer(Family family, InetSocketAddress server) {
        ...
        EndpointSelection.requireEndpoint(server);
        if (Family.of(server.getAddress()) != family
                || server.getAddress().isAnyLocalAddress()
                || server.getAddress().isMulticastAddress()) {
            throw new IllegalArgumentException("Numeric same-family STUN server required");
        }
    }

    private void requireOpen() {
        if (closed) {
            throw new IllegalStateException("Connectivity controller closed");
        }
    }

    @Override
    public synchronized void close() {
        if (closed && lanes.values().stream().allMatch(lane -> lane.monitor == null)) {
            return;
        }
        if (!closed) {
            candidateRevision = Math.incrementExact(candidateRevision);
