...
public final class MaintainedCandidatePublisher implements AutoCloseable {
    public record Publication(long mappingRevision, NativeCandidateSnapshot candidates, NativeCandidateSnapshot probeCandidates, Set<Integer> assistedFamilies,
                              Runnable current) {
    ...
    public MaintainedCandidatePublisher(EndpointSelection selection, EndpointConnectivityController controller, boolean assistedJoins) {
    ...
    MaintainedCandidatePublisher(EndpointSelection selection, EndpointConnectivityController controller, boolean assistedJoins, LongSupplier nanoTime) {
    ...
    public boolean needsStunServers() {
        return !selection.configured() && (controller != null || assistedJoins) && selection.socketFamilies().stream().anyMatch(f -> selection.publicCandidates(f).isEmpty());
    }
    ...
    public synchronized Publication refresh() {
        ...
        for (Family family : Family.values()) {
            ...
            direct.forEach(c -> probes.add(new NativeCandidateSnapshot.Candidate(c.endpoint(), NativeCandidateSnapshot.Type.HOST)));
            ...
            if (lane != null && lane.freshStunEndpoint().isPresent()) {
                ...
                probes.add(0, new NativeCandidateSnapshot.Candidate(endpoint, NativeCandidateSnapshot.Type.SRFLX));
            }
            if (!Objects.equals(mappings.get(family), nextMappings.get(family)))
                checks.entrySet().removeIf(e -> e.getValue().family() == number(family) && e.getKey().method().equals("warm_stun"));
            boolean failed = probes.stream().filter(c -> Family.of(c.endpoint().getAddress()) == family)
        ...
        // Test destinations survive withdrawal from player offers. Private addresses remain useful on LAN/VPN.
        probes.stream().filter(c -> assistedJoins || !failed(c.endpoint())).forEach(candidates::add);
        candidates.addAll(fallback);
        var probeCandidates = new NativeCandidateSnapshot(probes.stream().limit(32).toList());
        checks.entrySet().removeIf(e -> probes.stream().noneMatch(c -> c.endpoint().equals(e.getKey().target())));
        long capturedRevision = mappingRevision;
        return new Publication(mappingRevision, new NativeCandidateSnapshot(candidates.stream().limit(32).toList()), probeCandidates, assistedFamilies, () -> {
            requireOpen();
    ...
    public synchronized void reportDirectChecks(List<ConnectivityCheck> reports, long nowMillis) {
        ...
        for (var check : reports) {
            if (check.target() == null || check.checkedAt() > nowMillis || check.expiresAt() <= nowMillis
                    ...
                    || EndpointAddress.scope(check.target().getAddress()) != EndpointAddress.Scope.PUBLIC) continue;
            ...
            if (matches && (previous == null || check.checkedAt() > previous.checkedAt()
                    || check.checkedAt() == previous.checkedAt() && check.outcome() == ConnectivityOutcome.NOT_ESTABLISHED)) checks.put(key, check);
        }
    }
