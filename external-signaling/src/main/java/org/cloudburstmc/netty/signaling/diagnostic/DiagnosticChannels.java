...
final class DiagnosticChannels implements DataChannelCallback.Message {
                        ...
                        peer.createDataChannel(
                                ...
                                DataChannelInitSettings.DEFAULT.withReliability(
                                        DataChannelReliability.DEFAULT
                                                .withUnordered(index == 1)
                                                .withUnreliable(index == 1)
    ...
    boolean ready() {
        DataChannel channel = pending.poll();
        while (channel != null) {
            install(channel);
    ...
    private void install(DataChannel channel) {
        int index =
                channel.label().equals(label(0)) ? 0 : channel.label().equals(label(1)) ? 1 : -1;
        DataChannelReliability reliability = channel.reliability();
        if (index < 0
                || channels[index] != null
                || !channel.protocol().isEmpty()
                || reliability.isUnordered() != (index == 1)
                || reliability.isUnreliable() != (index == 1)
                || reliability.maxRetransmits() != 0
                || !reliability.maxPacketLifeTime().isZero()) {
            protocolFailed = true;
            throw invalid();
        }
    ...
    synchronized int receivedFrames() {
        return receivedFrames;
    }

    synchronized int receivedBytes() {
    ...
    synchronized void receive(boolean reliable, ByteBuffer bytes) {
        ...
        if (!reliable
                || bytes.remaining() != DiagnosticExchange.FRAME_BYTES
                || receivedFrames != 0) {
            protocolFailed = true;
            return;
        }
    ...
    void send(int channel, byte[] bytes) {
        if (protocolFailed || channel != 0 || bytes.length != DiagnosticExchange.FRAME_BYTES) {
            throw invalid();
        }
    ...
    private static String label(int index) {
        return index == 0 ? "ReliableDataChannel" : "UnreliableDataChannel";
    }
}
