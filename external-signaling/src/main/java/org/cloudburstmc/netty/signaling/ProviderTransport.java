...
public interface ProviderTransport {
    ...
    default boolean supportsAssistedJoins() {
        return false;
    }

    default CompletionStage<String> assistedJoin(
            org.cloudburstmc.netty.signaling.control.AssistedJoin join, Runnable requireCurrent) {
        return java.util.concurrent.CompletableFuture.failedFuture(
                new UnsupportedOperationException("Assisted joins unavailable"));
    }
    ...
    final class HostProfileSnapshot {
        ...
        public HostProfileSnapshot(
                JsonObject profile, long candidateRevision, Runnable requireCurrent) {
        ...
        public HostProfileSnapshot(
            ...
            if (probeCandidates.size() > 32 || !Set.of(4, 6).containsAll(assistedFamilies)) {
                throw new IllegalArgumentException("Connectivity snapshot bounds");
            }
            if (publicationVersion < 0) {
                throw new IllegalArgumentException("Publication version");
            }
            this.publicationVersion = publicationVersion;
            if (candidateRevision < 0 || candidateRevision > 9007199254740991L) {
                throw new IllegalArgumentException("Candidate revision");
            }
        ...
        public JsonArray probeCandidates() {
            return probeCandidates.deepCopy();
        }

        /** Families eligible for assistance when explicitly enabled by ProviderClient configuration. */
        public Set<Integer> assistedFamilies() {
            return assistedFamilies;
        }

        /** Zero means this adapter has no revisioned native capture. */
        public long candidateRevision() {
    ...
    default long candidatePublicationVersion() {
        return 0;
    }
    ...
    record ConnectivityCheck(
        ...
        public ConnectivityCheck(
                int family, ConnectivityOutcome outcome, long checkedAt, long expiresAt) {
            this(family, "defined", null, outcome, checkedAt, expiresAt);
        }

        public ConnectivityCheck {
            Objects.requireNonNull(outcome);
            if (region == null || !region.matches("[A-Za-z0-9_-]{1,32}")) {
                throw new IllegalArgumentException("Invalid connectivity region");
            }
            if (!Set.of("defined", "discovered", "warm_stun", "per_join").contains(method)
                    || target != null
                            && (target.isUnresolved()
                                    || target.getPort() < 1
                                    || (target.getAddress() instanceof java.net.Inet4Address
                                                    ? 4
                                                    : 6)
                                            != family)) {
    ...
    default CompletionStage<Void> reportConnectivityChecks(
            long candidateRevision, List<ConnectivityCheck> checks) {
        return java.util.concurrent.CompletableFuture.completedFuture(null);
    }

    /** Local opt-in, independent of player serving state. Incoming probes cannot configure their own authority. */
    default boolean supportsDiagnosticAdmission() {
        return false;
    }
    ...
    default CompletionStage<Void> configureDiagnostics(
    ...
    default CompletionStage<HostProfileSnapshot> captureHostProfile() {
        return hostProfile()
                .thenApply(
                        profile -> {
                            if (profile == null || profile.has("version")) {
                                throw new IllegalStateException(
                                        "Versioned host profiles require snapshot ownership");
                            }
