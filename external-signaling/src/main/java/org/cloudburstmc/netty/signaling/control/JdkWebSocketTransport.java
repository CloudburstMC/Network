...
public final class JdkWebSocketTransport implements AutoCloseable {
    ...
    public record Limits(int maxMessageBytes, int maxReceiveParts, int maxPendingSends,
                         long maxPendingBytes, Duration connectTimeout, Duration receiveTimeout,
                         Duration sendTimeout, Duration closeTimeout) {
        public Limits {
            if (maxMessageBytes < 1 || maxReceiveParts < 1 || maxPendingSends < 1 || maxPendingBytes < 1) {
                throw new IllegalArgumentException("WebSocket limits must be positive");
            }
            for (Duration timeout : List.of(connectTimeout, receiveTimeout, sendTimeout, closeTimeout)) {
    ...
    private JdkWebSocketTransport(Limits limits, String subprotocol, Executor receiverExecutor,
     ...
     */
    public static JdkWebSocketTransport connect(HttpClient client, URI endpoint, String subprotocol,
                                                Map<String, String> headers, Limits limits,
                                                Executor receiverExecutor, ScheduledExecutorService scheduler,
                                                Function<String, ? extends CompletionStage<?>> receiver) {
        ...
        return connect(client.newWebSocketBuilder(), endpoint, subprotocol, headers, limits,
                receiverExecutor, scheduler, receiver);
    }

    // Builder injection also lets tests hold sends pending deterministically without relying on TCP buffers.
    static JdkWebSocketTransport connect(WebSocket.Builder builder, URI endpoint, String subprotocol,
                                         Map<String, String> headers, Limits limits, Executor receiverExecutor,
                                         ScheduledExecutorService scheduler,
                                         Function<String, ? extends CompletionStage<?>> receiver) {
        Objects.requireNonNull(endpoint, "endpoint");
        if (!("wss".equalsIgnoreCase(endpoint.getScheme()) || "ws".equalsIgnoreCase(endpoint.getScheme()))
                || endpoint.getHost() == null || endpoint.getUserInfo() != null || endpoint.getFragment() != null) {
            throw new IllegalArgumentException("Expected a WebSocket endpoint without user-info or fragment");
        }
        JdkWebSocketTransport transport = new JdkWebSocketTransport(limits, subprotocol,
                receiverExecutor, scheduler, receiver);
        ...
        try {
            ...
            future.whenComplete((webSocket, failure) -> {
                if (failure != null) {
    ...
    public CompletionStage<Void> sendText(String text, Runnable requireCurrent) {
        ...
        synchronized (this.lock) {
            ...
            if (bytes > this.limits.maxMessageBytes()) {
                return CompletableFuture.failedFuture(new IOException("WebSocket message exceeds byte limit"));
            }
            if (this.sends.size() + (this.activeSend == null ? 0 : 1) >= this.limits.maxPendingSends()
                    || bytes > this.limits.maxPendingBytes() - this.pendingBytes) {
                return CompletableFuture.failedFuture(new IOException("WebSocket send capacity exhausted"));
            }
            this.sends.add(send);
    ...
    public CompletionStage<Close> closeGracefully() {
        try {
            synchronized (this.lock) {
                ...
                this.closeTimer = this.scheduler.schedule(
                        () -> this.fail(new TimeoutException("WebSocket close timed out")),
                        this.limits.closeTimeout().toNanos(), TimeUnit.NANOSECONDS);
            }
            this.pump();
        } catch (RuntimeException failure) {
            this.fail(failure);
    ...
    private void pump() {
        ...
        try {
            synchronized (this.lock) {
                if (this.terminal || this.socket == null || this.activeSend != null || this.closeSent) {
                    return;
                }
                ...
                if (send != null) {
                    ...
                    this.sendTimer = this.scheduler.schedule(() -> this.sendTimedOut(send),
                            this.limits.sendTimeout().toNanos(), TimeUnit.NANOSECONDS);
            ...
            if (send != null) {
                ...
                webSocket.sendClose(closing.statusCode(), closing.reason()).whenComplete((ignored, failure) -> {
                    if (failure != null) {
                        this.fail(failure);
                    }
                });
    ...
    private void terminate(Close close, Throwable failure, Send expectedSend, long expectedReceive) {
        List<Send> pending;
        WebSocket webSocket;
        CompletableFuture<WebSocket> handshake;
        synchronized (this.lock) {
            if (this.terminal || (expectedSend != null && this.activeSend != expectedSend)
                    || (expectedReceive >= 0 && (this.receiveTimer == null
            ...
            this.receiveTimer = this.sendTimer = this.closeTimer = null;
            this.incoming = null;
        ...
        Throwable sendFailure = failure == null ? new IOException("WebSocket peer closed") : failure;
                ...
                try {
                    Objects.requireNonNull(this.receiver.apply(text), "receiver stage")
                            .whenComplete((ignored, failure) -> {
                                ...
                                synchronized (this.lock) {
                                    if (this.terminal || this.receiveGeneration != generation) {
                                        return;
                                    }
                                    cancel(this.receiveTimer);
    ...
    private final class Listener implements WebSocket.Listener {
            ...
            try {
                ...
                synchronized (lock) {
                    ...
                    if (incoming == null) {
                        ...
                        // This also bounds time waiting for the application to accept the complete message.
                        receiveTimer = scheduler.schedule(() -> terminate(null,
                                        new TimeoutException("WebSocket receive timed out"), null, expected),
                                limits.receiveTimeout().toNanos(), TimeUnit.NANOSECONDS);
                    }
                    ...
                    incoming.append(data); // Own the characters before the JDK may reclaim its buffer.
                ...
                if (complete == null) {
                    requestNext(webSocket); // Demand counts fragments and control callbacks, not messages.
                } else {
        ...
        public CompletionStage<?> onPing(WebSocket webSocket, ByteBuffer message) {
            requestNext(webSocket); // The JDK sends the reciprocal pong; no buffer is retained here.
            return RECEIVED;
        }
