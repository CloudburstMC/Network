...
public final class DiagnosticBrowserHost {
    ...
    public static void main(String[] args) throws Exception {
        ...
        if (NetworkInterface.getByInetAddress(bind) == null
                || bind.isAnyLocalAddress()
                || bind.isMulticastAddress()) {
            throw new IllegalArgumentException("same_machine_only");
        }
        long now = System.currentTimeMillis(), runExpiry = integer(config, "runExpiresAt");
        if (runExpiry <= now || runExpiry > now + 35_000) {
        ...
        int port;
        try (DatagramSocket reservation = new DatagramSocket(new InetSocketAddress(bind, 0))) {
