...
/**
 * Test signaling source. Offer and issued token are NEVER delivered to the host.
 */
final class TestSignalingProvider {
    ...
    record Answer(String sdp, String token, String password) {
        @Override
    ...
    static String field(String sdp, String name) {
        return sdp.lines().filter(s -> s.startsWith("a=" + name + ":")).findFirst().orElseThrow()
    ...
    static Answer answer(String offer, String fingerprint, int port, long expiry, String audience,
                         boolean wrongClientFingerprint) throws Exception {
        return answer(offer, fingerprint, port, expiry, audience, wrongClientFingerprint, "K001", SECRET);
    }

    static Answer answer(String offer, String fingerprint, int port, long expiry, String audience,
                         boolean wrongClientFingerprint, String keyId, String secret) throws Exception {
        ...
        cipher.init(Cipher.ENCRYPT_MODE,
                new SecretKeySpec(hmac(utf8(secret), "nxs-stateless-aead-v1\0" + audience), "AES"),
                new GCMParameterSpec(128, nonce));
        cipher.updateAAD(utf8("nxs-stateless-admission-v1\0NXS1" + keyId + "\0" + audience + "\0" + ufrag));
        byte[] sealed = cipher.doFinal(claims.array());
        Arrays.fill(claims.array(), (byte) 0);
        var base64 = Base64.getEncoder().withoutPadding();
        String token = "NXS1" + keyId + base64.encodeToString(
                ByteBuffer.allocate(12 + sealed.length).put(nonce).put(sealed).array());
        String password = base64.encodeToString(
                Arrays.copyOf(hmac(utf8(secret), "nxs-stateless-ice-v1\0" + audience + "\0" + token), 24));
