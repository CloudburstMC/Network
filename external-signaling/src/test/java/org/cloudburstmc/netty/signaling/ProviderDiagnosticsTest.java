...
class ProviderDiagnosticsTest {
    ...
    static final class Transport extends ProviderClientTest.FakeTransport {
        ...
        volatile boolean withdrawn;
        Set<Integer> assistedFamilies = Set.of();
        volatile List<ProviderTransport.StunServer> stunServers = List.of();

        public CompletionStage<Void> configureStunServers(
                List<ProviderTransport.StunServer> servers) {
            stunServers = List.copyOf(servers);
            return CompletableFuture.completedFuture(null);
        }

        final List<List<ProviderTransport.ConnectivityCheck>> feedback =
                                            ...
                                            if (revision != captured
                                                    ...
                                                    || originalExpiry > 0
                                                            && System.currentTimeMillis()
                                                                    >= originalExpiry) {
                                                throw new IllegalStateException("stale snapshot");
                                            }
                                        });
                            });
        }
        ...
        public CompletionStage<Void> configureDiagnostics(
            ...
            if (policy.endpoints().stream()
                    .anyMatch(endpoint -> endpoint.candidateRevision() != revision)) {
                return CompletableFuture.failedFuture(
                        new IllegalStateException("stale installation"));
            }
    ...
    static final class Fixture implements AutoCloseable {
        ...
        Fixture() throws Exception {
            ...
            keys.init(store, "test".toCharArray());
        ...
        ProviderClient client(Path directory, Transport transport, boolean enabled, String method)
                throws Exception {
            var config =
                    new ProviderClient.Configuration(
                            URI.create(provider.origin),
                            "nxs-admission-v1",
                            "Diagnostics",
    ...
    @Test
    @Timeout(30)
    void successfulHeartbeatUsesExistingKeysAndProfileWithoutInstallationProtocol()
            throws Exception {
        try (var f = new Fixture()) {
            ...
            try {
                ...
                f.provider.heartbeatResponseHook =
                        () -> {
                            boolean advertised =
                                    f.provider.lastHeartbeat.has("extensions")
                                            && f.provider
                                                    .lastHeartbeat
                                                    .getAsJsonObject("extensions")
                                                    .getAsJsonObject(NAMESPACE)
                                                    .getAsJsonObject("data")
                                                    .get("diagnostics")
                                                    .getAsBoolean();
                            if (advertised) {
                                assertFalse(
                                        transport.policies.isEmpty(),
                                        "Opt-in preceded actual local installation");
                            }
                ...
                assertEquals(
                        registration.get("instanceId").getAsString(), policy.context().hostId());
                assertEquals(
                        registration.get("leaseGeneration").getAsLong(),
                        policy.context().generation());
                assertEquals(f.provider.origin, policy.context().providerOrigin());
                assertTrue(
                        policy.expiresAt() >= before
                                && policy.expiresAt() <= System.currentTimeMillis() + 300000);
                ...
                assertEquals(
                        Set.of("diagnostics", "candidateRevision", "method", "assistedFamilies"),
                        data.keySet());
                ...
                assertEquals(
                        policy.keys(),
                        transport.policies.get(transport.policies.size() - 1).keys());
    ...
    @Test
    @Timeout(30)
    void discoveryConfiguresStunAndPublishesTheMappingWithoutDuplicateProbeTargets()
            throws Exception {
        try (var f = new Fixture()) {
            f.provider.extensionMetadata =
                    JsonParser.parseString(
                                    "{\"dev.opencollab.nxs.connectivity\":{\"version\":1,\"critical\":false,\"data\":{\"stunServers\":[{\"host\":\"stun.example\",\"port\":3478}]}}}")
                            .getAsJsonObject();
            var transport = new Transport();
            transport.srflx = true;
            ...
            try {
                ...
                assertEquals(
                        List.of(new ProviderTransport.StunServer("stun.example", 3478)),
                        transport.stunServers);
                assertEquals(
                        1,
                        transport
                                .captureHostProfile()
                ...
                assertEquals(
                        Set.of(transport.expiry),
                        new HashSet<>(
                                transport
                                        .policies
                                        .get(transport.policies.size() - 1)
                                        .endpointExpiries()
                                        .values()));
            } finally {
                client.stop().toCompletableFuture().get(10, TimeUnit.SECONDS);
    ...
    @Test
    @Timeout(30)
    void withdrawnCandidatesRemainPublishedForRecoveryAndDisappearAfterRestoration()
            throws Exception {
        try (var f = new Fixture()) {
            var transport = new Transport();
            var client = f.client(directory, transport, true, "discovered");
            try {
                client.start().get(20, TimeUnit.SECONDS);
                transport.withdrawn = true;
                transport.publicationVersion++;
                client.readiness().get(10, TimeUnit.SECONDS);
                var data =
                        f.provider
                                .lastHeartbeat
                ...
                assertEquals(
                        1,
                        transport.policies.get(transport.policies.size() - 1).endpoints().size());
                transport.withdrawn = false;
                transport.publicationVersion++;
                client.readiness().get(10, TimeUnit.SECONDS);
                data =
                        f.provider
                                ...
                                .getAsJsonObject("data");
                assertFalse(
                        data.has("probeCandidates"),
                        "Matching lists need no duplicate publication");
            } finally {
                client.stop().toCompletableFuture().get(10, TimeUnit.SECONDS);
            }
        }
    ...
    @Test
    @Timeout(30)
    void failedHeartbeatDoesNotRenewPolicyAndNamespaceCannotBeForged() throws Exception {
        try (var f = new Fixture()) {
            var transport = new Transport();
            var client = f.client(directory, transport, true, "discovered");
            try {
                client.start().get(20, TimeUnit.SECONDS);
                int installed = transport.policies.size();
                long deadline = transport.deadline;
                f.provider.failHeartbeats = 100;
                assertThrows(
                        ExecutionException.class,
                        () -> client.readiness().get(10, TimeUnit.SECONDS));
                assertEquals(installed, transport.policies.size());
                assertEquals(deadline, transport.deadline);
                var forged =
                        JsonParser.parseString(
                ...
                assertThrows(
                        ...
                        () -> client.updateHeartbeatExtensions(forged));
            } finally {
                f.provider.failHeartbeats = 0;
                client.stop().toCompletableFuture().get(10, TimeUnit.SECONDS);
            }
        }
    }
    ...
    @Test
    @Timeout(30)
    void candidateReplacementDuringHeartbeatCannotInstallOldPolicy() throws Exception {
        try (var f = new Fixture()) {
            var transport = new Transport();
            var client = f.client(directory, transport, true, "defined");
            ...
            try {
                assertThrows(
                        ExecutionException.class, () -> client.start().get(20, TimeUnit.SECONDS));
                assertTrue(transport.policies.isEmpty());
                assertTrue(transport.disabled > 0);
            } finally {
                client.stop().toCompletableFuture().get(10, TimeUnit.SECONDS);
            }
        }
    ...
    @Test
    @Timeout(30)
    void diagnosticOptInSupportsExistingV1ReflexiveCandidateShape() throws Exception {
        for (boolean enabled : List.of(false, true)) {
            try (var f = new Fixture()) {
                var transport = new Transport();
                transport.srflx = enabled;
                var client =
                        f.client(
                                directory.resolve(Boolean.toString(enabled)),
                                transport,
                                enabled,
                                "discovered");
                try {
    ...
    @Test
    @Timeout(30)
    void disabledAssistanceCannotAdvertiseAnAssistedOnlyDiagnosticPolicy() throws Exception {
        try (var f = new Fixture()) {
            var transport = new Transport();
            transport.empty = true;
            transport.assistedFamilies = Set.of(4);
            var client = f.client(directory, transport, true, "discovered");
            try {
                client.start().get(20, TimeUnit.SECONDS);
                assertTrue(transport.policies.isEmpty());
    ...
    @Test
    @Timeout(30)
    void maintainedExpiryAndMethodUseTheOriginalNativeObservation() throws Exception {
        try (var f = new Fixture()) {
            var transport = new Transport();
            transport.srflx = true;
            long original = System.currentTimeMillis() + 180000;
            transport.expiry = original;
            var client = f.client(directory, transport, true, "discovered");
            try {
                client.start().get(20, TimeUnit.SECONDS);
                var data =
                ...
                for (var policy : transport.policies) {
                    assertEquals(
                            Set.of(original), new HashSet<>(policy.endpointExpiries().values()));
    ...
    @Test
    @Timeout(30)
    void materialReplacementAndWithdrawalWakeLongHeartbeatWithoutDiagnostics() throws Exception {
        try (var f = new Fixture()) {
    ...
    @Test
    @Timeout(30)
    void freshnessOnlyChangesAreCoalescedButRetainTheirNewOriginalExpiry() throws Exception {
        try (var f = new Fixture()) {
            f.provider.checkInMillis = 120000;
            var transport = new Transport();
            transport.srflx = true;
            ...
            try {
                client.start().get(20, TimeUnit.SECONDS);
                int before = f.provider.heartbeats;
                transport.expiry += 10000;
                transport.publicationVersion++;
                Thread.sleep(2200);
                assertEquals(
                        before,
                        f.provider.heartbeats,
                        "A fresh same-mapping observation must not send every timer tick");
                client.readiness().get(10, TimeUnit.SECONDS);
                assertEquals(
                        transport.expiry,
                        f.provider
    ...
    @Test
    @Timeout(30)
    void feedbackKeepsOriginalTimesAndIgnoresExpiredFutureAndWrongRevisionChecks()
            throws Exception {
        try (var f = new Fixture()) {
            var transport = new Transport();
            var client = f.client(directory, transport, false, "discovered");
            try {
                ...
                f.provider.extensionMetadata = new JsonObject();
                f.provider.extensionMetadata.add(NAMESPACE, extension);
                client.readiness().get(10, TimeUnit.SECONDS);
                assertEquals(
                        List.of(
                ...
                assertEquals(
                        ProviderTransport.ConnectivityOutcome.UNAVAILABLE,
                        transport.feedback.get(1).get(0).outcome());
    ...
    @Test
    @Timeout(30)
    void aRetryCannotSendAProfileWhoseNativeMappingChanged() throws Exception {
        try (var f = new Fixture()) {
            var transport = new Transport();
            var client = f.client(directory, transport, false, "discovered");
            try {
    ...
    @Test
    @Timeout(30)
    void shortDiagnosticLeaseDoesNotTurnTheTimerIntoAHeartbeatLoop() throws Exception {
        try (var f = new Fixture()) {
            f.provider.checkInMillis =
                    10000; // The fixture's lease is only 40s, less than a fixed 60s renewal lead.
            var transport = new Transport();
            var client = f.client(directory, transport, true, "defined");
            try {
                client.start().get(20, TimeUnit.SECONDS);
