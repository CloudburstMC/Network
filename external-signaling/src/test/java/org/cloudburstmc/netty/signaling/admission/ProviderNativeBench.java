...
public final class ProviderNativeBench {
    ...
    private static final class LoopbackAssistedTransport implements ProviderTransport {
        ...
        LoopbackAssistedTransport(NativeProviderTransport transport, InetSocketAddress bind) {
            if (!bind.getAddress().isLoopbackAddress()) {
                throw new IllegalArgumentException("Loopback fixture only");
            }
                    ...
                    .thenApply(
                            original -> {
                                ...
                                return new HostProfileSnapshot(
                                        profile,
                                        original.candidateRevision(),
                                        original.publicationVersion(),
                                        new JsonArray(),
                                        Set.of(family),
        ...
        public long candidatePublicationVersion() {
            return transport.candidatePublicationVersion();
        }
                            ...
                            snapshot -> {
                                ...
                                return join.diagnostic()
                                        ? transport
                                                .channel()
                                                .assistDiagnostic(
                                                        join, guard, Map.of(), Map.of(family, bind))
                                        : transport
                                                .channel()
                                                .assist(
                                                        join,
                                                                                        ...
                                                                                        DiagnosticHostPolicy
                                                                                                ...
                                                                                                .assisted(
                                                                                                        ...
                                                                                                        snapshot
                                                                                                                .candidateRevision())))) {
                                                            throw new IllegalStateException(
                                                                    "Fixture diagnostic authority changed");
                                                        }
                                                        long expiry =
                                                                Math.min(
                                                                        policy.expiresAt(),
        ...
        public CompletionStage<Void> disableDiagnostics() {
            if (diagnostics != null) {
                diagnostics.retainEndpoints(Set.of());
            }
        ...
        public CompletionStage<Void> installTicketKeys(List<TicketKey> keys) {
            return transport.installTicketKeys(keys);
        }

        @Override
        public List<JsonObject> pollEvents() {
            return transport.pollEvents();
        }
        ...
        try {
            ...
            nativeHost =
                    NativeProviderTransport.open(
                                    ...
                                    bind,
                                    advertised,
                                    state.resolve("host-cert.pem"),
                                    state.resolve("host-key.pem"),
                                    new AdmissionGate.Limits(4, 8, 2, 10_000))
                            .toCompletableFuture()
                            .get(10, TimeUnit.SECONDS);
            ProviderTransport transport =
                    assisted ? new LoopbackAssistedTransport(nativeHost, bind) : nativeHost;
            provider =
                    new ProviderClient(
                            ...
                            () ->
                                    new ServerStatus(
                                            "Automatic native server", "Integration", 0, 4, 0),
            ...
            while (!Files.exists(stop)) {
                ...
                emit(
                        ...
                        Map.of(
                                "admission",
                                endpoint.admissionStats(),
                                "native",
                                endpoint.nativeStats(),
                                "nativeCreationAttempts",
                                NativeDiagnostics.creationAttempts().orElse(-1),
                                "hostCreations",
                                endpoint.creationAttempts(),
                                "deliveredChannels",
                                delivered.get(),
                                "controlCarrier",
                                provider.lastControlCarrier(),
                                "identityVerified",
                                identityVerified.get()));
                Thread.sleep(100);
            }
