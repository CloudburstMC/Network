...
class NativeAdmissionIntegrationTest {
    ...
    void transportPublishesBindingBeforeLoginAndRejectsADifferentKey() throws Exception {
        ...
        validator.installKeys(List.of(
                new StatelessAdmissionValidator.TicketKey("K001", TestSignalingProvider.SECRET)));
        ...
        try {
            ...
            for (boolean matching : new boolean[]{true, false}) {
                byte[] loginKey = matching ? Base64.getDecoder().decode(TestSignalingProvider.IDENTITY_CPK)
                ...
                try (var client = PeerConnection.createPeer(PeerConnectionConfiguration.DEFAULT
                    ...
                    var answer = TestSignalingProvider.answer(client.localDescription(), id.fingerprint(), 49201,
    ...
    void rotatedEpochsAcceptBeforeOriginalCutoffButOnlyNewEpochAtAndAfterIt() throws Exception {
        ...
        validator.installKeys(
                List.of(
                        new StatelessAdmissionValidator.TicketKey(
                                "K001", TestSignalingProvider.SECRET, 0, cutoff),
        ...
        var endpoint =
                new NativeAdmissionServerChannel(
                        id,
                        (request, ignored) -> validator.validate(request, now.get()),
        ...
        try {
            ...
            for (long instant : new long[] {cutoff - 1, cutoff, cutoff + 1}) {
                ...
                for (String keyId : List.of("K001", "K002")) {
                    ...
                    // Each attempt has a distinct token/source tuple; an already admitted peer
                    // cannot
                    ...
                    var answer =
                            TestSignalingProvider.answer(
                                    ...
                                    keyId,
                                    keyId.equals("K001")
                                            ? TestSignalingProvider.SECRET
                                            : newSecret);
                    byte[] request =
                            nominatedBinding(answer.token() + ":" + ufrag, answer.password());
                    ...
                    try (var socket = new DatagramSocket(new InetSocketAddress("127.0.0.1", 0))) {
                        ...
                        if (accepted) {
                            ...
                            assertEquals(
                                    0x0101,
                                    Short.toUnsignedInt(
                                            ByteBuffer.wrap(response.getData()).getShort()));
                            assertArrayEquals(
                                    Arrays.copyOfRange(request, 8, 20),
                            ...
                            assertThrows(
                                    SocketTimeoutException.class, () -> socket.receive(response));
                            await(() -> endpoint.admissionStats().invalid() > invalidBefore);
                        }
                        assertEquals(
                                expectedCreations,
                                endpoint.creationAttempts(),
                                keyId + " at " + instant);
                    }
                }
            }
    ...
    void dualStackWildcardAcceptsBothFamiliesAndRetainsSingleTicketOwnership() throws Exception {
        ...
        try {
                ...
                try (var socket = new DatagramSocket(new InetSocketAddress(destination.getAddress(), 0));
                     ...
                     var client = PeerConnection.createPeer(
                             PeerConnectionConfiguration.DEFAULT.withDisableAutoNegotiation(true)
                                     .withBindAddress(destination.getAddress()), Runnable::run)) {
                    ...
                    socket.receive(response);
                    assertEquals(destination.getAddress(), response.getAddress());
                    ...
                    assertThrows(SocketTimeoutException.class,
                            () -> duplicate.receive(new DatagramPacket(new byte[2048], 2048)));
                    assertEquals(creations, endpoint.creationAttempts(),
                            "An alternate family cannot allocate a second peer with the same ticket");
                }
            }
            assertEquals(2, endpoint.creationAttempts());
            ...
            if (host != null) {
                host.close().toCompletableFuture().get(10, TimeUnit.SECONDS);
            }
            group.shutdownGracefully(0, 1, TimeUnit.SECONDS).sync();
        }
        try (var socket = new DatagramSocket(new InetSocketAddress("::", port))) {
    ...
    @Test
    @Timeout(40)
    void clientsOfEitherFamilyOpenBothDataChannelsFromTheSameCandidateList() throws Exception {
        var id = identity();
        var group = new DefaultEventLoopGroup(1);
        int port = 49187;
        NativeProviderTransport host = null;
        try {
            ...
            for (String ip : List.of("127.0.0.1", "::1")) {
                try (var client = PeerConnection.createPeer(
                    ...
                    client.setLocalDescription("offer", ip.equals("::1") ? "clientWithIpv6" : "clientWithIpv4",
                            "p".repeat(32));
                    var answer = TestSignalingProvider.answer(client.localDescription(), id.fingerprint(), port,
                            System.currentTimeMillis() + 30_000, NativeProviderTransport.audience(incarnation), false);
                    String sdp = answer.sdp().replace("a=candidate:1 1 UDP 2130706431 127.0.0.1",
                                    "a=candidate:2 1 UDP 2130706175 127.0.0.1")
                            ...
                            events.stream()
                                    .filter(
                                            e ->
                                                    e.get("stage")
                                                            .getAsString()
                                                            .equals("ticket.data_channels_open"))
                                    .findFirst()
                                    .orElseThrow();
                    assertEquals(
                            InetAddress.getByName(ip),
                            InetAddress.getByName(connected.get("remoteAddress").getAsString()));
                    assertEquals(
                            client.selectedCandidatePair().local().getPort(),
                            connected.get("remotePort").getAsInt());
                    assertTrue(
                            events.stream()
                                    .filter(
                                            e ->
                                                    e.get("stage")
                                                            .getAsString()
                                                            .equals("ticket.ice_seen"))
                                    .allMatch(e -> !e.has("remoteAddress") && !e.has("remotePort")),
                            "No selected endpoint is invented before connection");
                    assertTrue(client.closeAndAwait(Duration.ofSeconds(5)));
                }
            }
    ...
    NativeHostIdentity identity() throws Exception {
        ...
        Process p = new ProcessBuilder("openssl", "req", "-x509", "-newkey", "ec", "-pkeyopt",
                "ec_paramgen_curve:prime256v1", "-nodes", "-keyout", key.toString(), "-out", cert.toString(), "-days",
                "1", "-subj", "/CN=public-test-only").redirectErrorStream(true)
    ...
    static void rakPing(int port) throws Exception {
        try (var socket = new DatagramSocket()) {
            socket.setSoTimeout(2000);
            byte[] ping =
                    ByteBuffer.allocate(33).put((byte) 1).putLong(12345).put(RakConstants.DEFAULT_UNCONNECTED_MAGIC)
                            .putLong(42).array();
            socket.send(new DatagramPacket(ping, ping.length, InetAddress.getByName("127.0.0.1"), port));
            byte[] reply = new byte[2048];
            var packet = new DatagramPacket(reply, reply.length);
            socket.receive(packet);
            assertEquals(port, packet.getPort());
            assertEquals(0x1c, reply[0]);
            assertEquals(12345, ByteBuffer.wrap(reply).getLong(1));
        }
    }

    @Test
    @Timeout(20)
    void wildcardBindPublishesExplicitCandidateAndAcceptsIngress() throws Exception {
        var id = identity();
        var group = new DefaultEventLoopGroup(1);
        ...
        try {
            ...
            host.installTicketKeys(List.of(new ProviderTransport.TicketKey(
                    "K001", TestSignalingProvider.SECRET, 0, Long.MAX_VALUE))).toCompletableFuture().get();
            var profile = host.hostProfile().toCompletableFuture().get();
            ...
            assertEquals("nethernet.stateless-admission.v1",
                    profile.getAsJsonObject("statelessAdmission").get("capability").getAsString());
            try (var socket = new DatagramSocket()) {
                byte[] packet = new byte[40];
                socket.send(new DatagramPacket(packet, packet.length, advertised));
            }
            var endpoint = host.channel();
        ...
        try (var socket = new DatagramSocket(bind)) {
            assertEquals(49189, socket.getLocalPort());
        }
    }

    @Test
    @Timeout(20)
    void firstAuthenticatedDatagramGetsMatchingResponseWithoutRetry() throws Exception {
        var id = identity();
        ...
        try (var socket = new DatagramSocket(new InetSocketAddress(loopback, 0));
            ...
            assertEquals(loopback, response.getAddress());
            assertEquals(port, response.getPort());
            assertEquals(0x0101, Short.toUnsignedInt(ByteBuffer.wrap(bytes).getShort()));
            assertArrayEquals(Arrays.copyOfRange(request, 8, 20), Arrays.copyOfRange(bytes, 8, 20));
            ...
            assertEquals(1, endpoint.nativeStats()[5]);
            ...
            assertEquals(1, validations.get());
            assertEquals(1, endpoint.nativeStats()[5]);
            System.out.printf(Locale.ROOT, "first-stun PASS requestsSent=1 matchingSuccess=true responseMs=%.3f%n",
                    elapsedMs);
        } finally {
            if (endpoint.isRegistered()) {
                endpoint.close().awaitUninterruptibly();
            } else {
                endpoint.unsafe().closeForcibly();
            }
            endpoint.termination().toCompletableFuture().get(6, TimeUnit.SECONDS);
            group.shutdownGracefully(0, 1, TimeUnit.SECONDS).sync();
        }
    ...
    @Test
    @Timeout(20)
    void expiredDecisionCreatesNoPeerAndReleasesItsReservation() throws Exception {
        var id = identity();
        var loopback = InetAddress.getByName("127.0.0.1");
        int port = 49200;
        var validator = new StatelessAdmissionValidator(TestSignalingProvider.AUDIENCE, 60_000);
        ...
        try (var socket = new DatagramSocket(new InetSocketAddress(loopback, 0));
            ...
            var answer = TestSignalingProvider.answer(client.localDescription(), id.fingerprint(), port,
                    System.currentTimeMillis() + 2000, TestSignalingProvider.AUDIENCE, false);
    ...
    @Test
    @Timeout(60)
    void tokenLengthBoundsAndClientFingerprintAreEnforcedByRealTransport() throws Exception {
        var id = identity();
    ...
    @Test
    @Timeout(45)
    void noControlLazyJoinBothChannelsReplayAndCleanup() throws Exception {
        var id = identity();
        var loopback = InetAddress.getByName("127.0.0.1");
        int port = 49190;
        var validator = new StatelessAdmissionValidator(TestSignalingProvider.AUDIENCE, 60_000);
                    ...
                    protected void initChannel(AdmittedNetherNetChildChannel ch) {
                        ...
                        ch.pipeline().addLast(new SimpleChannelInboundHandler<ByteBuf>() {
                            ...
                            public void userEventTriggered(ChannelHandlerContext ctx, Object event) {
                                if (event instanceof NetherNetPacket.Delivery d) {
                                    reliable = d.reliable();
                                }
                            }
                            ...
                            protected void channelRead0(ChannelHandlerContext ctx, ByteBuf data) {
                                ...
                                ByteBuf echo =
                                        ctx.alloc().buffer(data.readableBytes() + 3).writeZero(3).writeBytes(data);
                                echo.skipBytes(3);
        ...
        try {
            bootstrap.bind(new InetSocketAddress(loopback, port)).sync();
            rakPing(49191);
            assertEquals(0, endpoint.nativeStats()[2]);
            assertEquals(0, endpoint.admissionStats().claims());
            ...
            try (PeerConnection client = PeerConnection.createPeer(
                    PeerConnectionConfiguration.DEFAULT.withDisableAutoNegotiation(true).withBindAddress(loopback),
                    Runnable::run)) {
                ...
                for (int index = 0; index < 2; index++) {
                    ...
                    String label = reliable ? "ReliableDataChannel" : "UnreliableDataChannel";
                    ...
                    dc.onOpen.register(d -> {
                        ...
                        for (int i = 0; i < chunks; i++) {
                            int count = Math.min(9999, payload.length - i * 9999);
                            ByteBuffer frame = ByteBuffer.allocateDirect(count + 1);
                            frame.put((byte) (chunks - i - 1)).put(payload, i * 9999, count).flip();
                            d.sendMessage(frame);
                        }
                    });
                }
                ...
                var answer = TestSignalingProvider.answer(client.localDescription(), id.fingerprint(), port,
                        System.currentTimeMillis() + 30_000, TestSignalingProvider.AUDIENCE, false);
                var expired = TestSignalingProvider.answer(client.localDescription(), id.fingerprint(), port,
                        System.currentTimeMillis() - 1_000, TestSignalingProvider.AUDIENCE, false);
                var wrongHost = TestSignalingProvider.answer(client.localDescription(), id.fingerprint(), port,
                        System.currentTimeMillis() + 30_000, "sig_fixture/gs_two/test_boot_001", false);
                String altered = answer.token().substring(0, 80) + (answer.token().charAt(80) == 'A' ? 'B' : 'A')
                        + answer.token().substring(81);
                List<byte[]> rejectedPackets = List.of(
                        StatelessAdmissionValidatorTest.binding(expired.token() + ":clientFixtureUf",
                                expired.password()),
                        StatelessAdmissionValidatorTest.binding(wrongHost.token() + ":clientFixtureUf",
                                wrongHost.password()),
                        StatelessAdmissionValidatorTest.binding(altered + ":clientFixtureUf", answer.password()),
                        StatelessAdmissionValidatorTest.binding(answer.token() + ":clientFixtureUf",
                ...
                try (var replay = new DatagramSocket()) {
                    byte[] packet = StatelessAdmissionValidatorTest.binding(answer.token() + ":clientFixtureUf",
                            answer.password());
                ...
                assertEquals(0, child.get().queuedFrames());
                assertEquals(0, child.get().retainedAssemblyBytes());
            }
            endpoint.close().sync();
            endpoint.termination().toCompletableFuture().get(5, TimeUnit.SECONDS);
            try (var reuse = new DatagramSocket(new InetSocketAddress(loopback, port))) {
                assertEquals(port, reuse.getLocalPort());
            }
            System.out.println(
            ...
            rakGroup.shutdownGracefully(0, 2, TimeUnit.SECONDS).sync();
        }
    }

    @Test
    @Timeout(30)
    void providerBoundaryPublishesFreshBootIdentityWithoutClientState() throws Exception {
        var id = identity();
        var group = new DefaultEventLoopGroup(1);
        ...
        try {
            ...
            transport.installTicketKeys(
                    List.of(new ProviderTransport.TicketKey("K001",
                                    TestSignalingProvider.SECRET, 0, System.currentTimeMillis() + 60_000),
            ...
            assertTrue(transport.hostProfile().toCompletableFuture().isCompletedExceptionally());
            assertFalse(transport.channel().isServing());
    ...
    @Test
    @Timeout(40)
    void simultaneousClientsRespectNativeCapacityUntilActualTeardown() throws Exception {
        var id = identity();
        var group = new DefaultEventLoopGroup(2);
        var clients = new ArrayList<PeerConnection>();
        var children = new CopyOnWriteArrayList<AdmittedNetherNetChildChannel>();
        var validator = new StatelessAdmissionValidator(TestSignalingProvider.AUDIENCE, 60000);
        validator.installKeys(
                List.of(new StatelessAdmissionValidator.TicketKey("K001", TestSignalingProvider.SECRET)));
        ...
        try {
            ...
            var answers = new ArrayList<TestSignalingProvider.Answer>();
            var opens = new AtomicIntegerArray(3);
            for (int i = 0; i < 3; i++) {
                final int index = i;
                var client = PeerConnection.createPeer(
                        PeerConnectionConfiguration.DEFAULT.withDisableAutoNegotiation(true)
                                .withBindAddress(InetAddress.getByName("127.0.0.1")), Runnable::run);
                clients.add(client);
                for (boolean reliable : new boolean[]{true, false}) {
                    var dc = client.createDataChannel(reliable ? "ReliableDataChannel" : "UnreliableDataChannel",
                ...
                answers.add(TestSignalingProvider.answer(client.localDescription(), id.fingerprint(), 49198,
                        System.currentTimeMillis() + 30000, TestSignalingProvider.AUDIENCE, false));
            }
            OptionalLong before = NativeDiagnostics.creationAttempts();
            ...
            closing.close().sync();
            closing.nativeTermination().toCompletableFuture().get(5, TimeUnit.SECONDS);
            // Third client's normal ICE retries can claim the released slot; used tokens remain tombstoned.
            await(() -> {
                assertTrue(endpoint.nativeStats()[2] <= 2);
                assertTrue(endpoint.liveNativePeers() <= 2);
            ...
            System.out.println(
                    "native-capacity PASS simultaneousClients=2 thirdRetriesAfterTeardown=true maxNativePeers=2");
        } finally {
