...
class NativeMaintainedCandidateTest {
    ...
    private static final List<ProviderTransport.TicketKey> KEYS =
            List.of(new ProviderTransport.TicketKey("K001", TestSignalingProvider.SECRET));

    @Test
    @Timeout(20)
    void withdrawnEndpointRetainsDiagnosticAuthorityForRecovery() throws Exception {
        try (var host = new Host(Map.of(), List.of("8.8.8.8"))) {
            ...
            var endpoint =
                    new DiagnosticHostPolicy.Endpoint(
                            4,
                            DiagnosticAdmissionCodec.address(4, "8.8.8.8"),
                            host.port,
                            before.candidateRevision());
            var policy =
                    new DiagnosticHostPolicy(
                            context,
                            List.of(
                                    new DiagnosticAdmissionCodec.Key(
                                            "K001", TestSignalingProvider.SECRET, 0, expires)),
                            Set.of(endpoint),
                            expires);
            host.transport
                    .configureDiagnostics(policy)
                    .toCompletableFuture()
                    .get(5, TimeUnit.SECONDS);
            long now = System.currentTimeMillis();
            var target = new InetSocketAddress("8.8.8.8", host.port);
            host.transport
                    .reportConnectivityChecks(
                            before.candidateRevision(),
                            List.of(
                                    new ProviderTransport.ConnectivityCheck(
                                            ...
                                            "discovered",
                                            target,
                                            ProviderTransport.ConnectivityOutcome.NOT_ESTABLISHED,
                                            now - 1,
                                            now + 30_000)))
                    .toCompletableFuture()
                    .get();
            var withdrawn = host.transport.captureHostProfile().toCompletableFuture().get();
            assertTrue(withdrawn.profile().getAsJsonArray("candidates").isEmpty());
            assertEquals(1, withdrawn.probeCandidates().size());
            assertEquals(before.candidateRevision(), withdrawn.candidateRevision());
            host.transport
                    .configureDiagnostics(policy)
                    .toCompletableFuture()
                    .get(5, TimeUnit.SECONDS);
                    ...
                    .reportConnectivityChecks(
                            ...
                            List.of(
                                    new ProviderTransport.ConnectivityCheck(
                                            ...
                                            "discovered",
                                            target,
                                            ProviderTransport.ConnectivityOutcome.ESTABLISHED,
    ...
    private final class Host implements AutoCloseable {
        ...
        Host(Map<EndpointSelection.Family, InetSocketAddress> servers) throws Exception {
            this(servers, List.of());
        }

        Host(Map<EndpointSelection.Family, InetSocketAddress> servers, List<String> direct)
                throws Exception {
            var helper = new NativeAdmissionIntegrationTest();
            helper.directory = directory;
            identity = helper.identity();
            try (var socket = new DatagramSocket(new InetSocketAddress("::", 0))) {
                            ...
                            .childHandler(
                                    new ChannelInitializer<AdmittedNetherNetChildChannel>() {
                                        ...
                                        protected void initChannel(
                                                AdmittedNetherNetChildChannel channel) {
                                            channel.pipeline()
                                                    .addLast(
            ...
            transport =
                    NativeProviderTransport.openMaintained(
                                    bootstrap,
                                    EndpointSelection.select(
                                            ...
                                            direct.stream()
                                                    .map(
                                                            ip ->
                                                                    new EndpointSelection.Candidate(
        ...
        String audience() throws Exception {
            return NativeProviderTransport.audience(
                    transport
                            .hostProfile()
                            .toCompletableFuture()
                            .get()
        ...
        public void close() throws Exception {
            try {
                transport.close().toCompletableFuture().get(8, TimeUnit.SECONDS);
            } finally {
    ...
    private final class ConnectedPlayer implements AutoCloseable {
        ...
        final AtomicInteger reliableEchoes = new AtomicInteger(),
                unreliableEchoes = new AtomicInteger();
            ...
            player =
                    PeerConnection.createPeer(
                            PeerConnectionConfiguration.DEFAULT
                                    ...
                                    .withBindAddress(InetAddress.getByName(ip)),
                            Runnable::run);
                    ...
                    DataChannelCallback.Message.handleBinary(
                            (channel, bytes) -> {
                                if (bytes.remaining() == 2
                                        && bytes.get() == 0
                                        && bytes.get() == 42) {
                                    reliableEchoes.incrementAndGet();
                                }
                            }));
            unreliable.onMessage.register(
                    DataChannelCallback.Message.handleBinary(
                            (channel, bytes) -> {
                                if (bytes.remaining() == 2
                                        && bytes.get() == 0
                                        && bytes.get() == 42) {
                                    unreliableEchoes.incrementAndGet();
                                }
                            }));
            player.setLocalDescription(
                    "offer",
                    ip.equals("::1") ? "snapshotPlayer6" : "snapshotPlayer4",
                    "p".repeat(32));
            var answer =
                    TestSignalingProvider.answer(
                            ...
                            host.port,
                            System.currentTimeMillis() + ticketMillis,
                            host.audience(),
                            false);
            player.setRemoteDescription(
                    answer.sdp().replace("127.0.0.1", ip), SessionDescriptionType.ANSWER);
            NativeAdmissionIntegrationTest.await(() -> reliable.isOpen() && unreliable.isOpen());
        }

        void exchange() throws Exception {
            int expectedReliable = reliableEchoes.get() + 1,
                    expectedUnreliable = unreliableEchoes.get() + 1;
            ...
            NativeAdmissionIntegrationTest.await(
                    () ->
                            reliableEchoes.get() == expectedReliable
                                    && unreliableEchoes.get() == expectedUnreliable);
    ...
    private static void maintainedResponses(DatagramSocket server, int gameplayPort)
            throws Exception {
        server.setSoTimeout(20000);
        for (int sequence = 1; sequence <= 3; sequence++) {
            byte[] bytes = new byte[1024];
            var request = new DatagramPacket(bytes, bytes.length);
            ...
            input.getShort();
            assertEquals(0x2112a442, input.getInt());
            var transaction = new byte[12];
            input.get(transaction);
            byte[] address =
                    InetAddress.getByName(
                                    server.getLocalAddress() instanceof Inet4Address
                                            ? "8.8.8.8"
                                            : "2606:4700:4700::1001")
                            .getAddress();
            var response = ByteBuffer.allocate(address.length == 4 ? 32 : 44);
            response.putShort((short) 0x101)
                    .putShort((short) (response.capacity() - 20))
                    .putInt(0x2112a442)
                    .put(transaction)
                    .putShort((short) 0x20)
                    .putShort((short) (address.length + 4))
                    .put((byte) 0)
                    .put((byte) (address.length == 4 ? 1 : 2))
                    .putShort((short) ((43000 + (sequence == 3 ? 1 : 0)) ^ 0x2112));
            for (int i = 0; i < address.length; i++) {
                response.put((byte) (address[i] ^ bytes[4 + i]));
            }
            server.send(
                    new DatagramPacket(
                            response.array(), response.capacity(), request.getSocketAddress()));
        }
    }

    private static ProviderTransport.HostProfileSnapshot awaitMaintained(
            NativeProviderTransport transport, int port) throws Exception {
        ...
        do {
            ...
            if (candidates.size() == 2
                    && candidates.asList().stream()
                            .allMatch(
                                    value -> {
                                        var candidate = value.getAsJsonObject();
                                        assertFalse(
                                                candidate.has("expiresAt"),
                                                "STUN uses the existing candidate wire shape");
                                        return candidate.get("port").getAsInt() == port
                                                && candidate
                                                        .get("type")
                                                        .getAsString()
                                                        .equals("srflx");
    ...
    void maintainedMuxRenewsAndRemapsWithoutControlTrafficOrReinstallingPeers() throws Exception {
        ...
        try (var stun4 = new DatagramSocket(new InetSocketAddress("127.0.0.1", 0));
                var stun6 = new DatagramSocket(new InetSocketAddress("::1", 0));
                var host =
                        new Host(
                                Map.of(
                                        EndpointSelection.Family.IPV4,
                                        (InetSocketAddress) stun4.getLocalSocketAddress(),
                                        EndpointSelection.Family.IPV6,
                                        (InetSocketAddress) stun6.getLocalSocketAddress()),
                                List.of("10.0.0.8", "fd00::8"))) {
            var v4 =
                    responders.submit(
                            () -> {
                                maintainedResponses(stun4, host.port);
                                return null;
                            });
            var v6 =
                    responders.submit(
                            () -> {
                                maintainedResponses(stun6, host.port);
                                return null;
                            });
            host.transport.installTicketKeys(KEYS).toCompletableFuture().get();
            ...
            assertEquals(
                    2,
                    first.profile().getAsJsonArray("candidates").size(),
                    "Mapping publication needs no regional proof");
            try (var player4 = new ConnectedPlayer(host, "127.0.0.1", 55000);
                    var player6 = new ConnectedPlayer(host, "::1", 55000)) {
                player4.exchange();
                player6.exchange();
                ...
                for (var value : remapped.profile().getAsJsonArray("candidates")) {
                    ...
                    var target =
                            new InetSocketAddress(
                                    ...
                                    candidate.get("port").getAsInt());
                    failure.add(
                            new ProviderTransport.ConnectivityCheck(
                                    target.getAddress() instanceof Inet4Address ? 4 : 6,
                                    "warm_stun",
                                    target,
                                    ProviderTransport.ConnectivityOutcome.NOT_ESTABLISHED,
                                    failedAt,
                ...
                host.transport
                        .reportConnectivityChecks(remapped.candidateRevision(), failure)
                        .toCompletableFuture()
                        .get();
                var after = host.transport.captureHostProfile().toCompletableFuture().get();
                assertEquals(
                        remapped.candidateRevision(),
                        after.candidateRevision(),
                        "Offer decisions must preserve recovery probe authority");
                assertTrue(after.publicationVersion() > remapped.publicationVersion());
                assertEquals(
                        2,
                        after.profile().getAsJsonArray("candidates").size(),
                        "Failed public mappings are withheld; LAN/VPN fallback remains");
            ...
            v4.get(1, TimeUnit.SECONDS);
            v6.get(1, TimeUnit.SECONDS);
            System.out.println(
                    "maintained-native PASS families=2 sameGameplayPort=true observationsPerFamily=3 controlTraffic=false peersPreserved=2 syntheticPublicMappings=true");
        } finally {
        ...
        try (var stun = new DatagramSocket(new InetSocketAddress("127.0.0.1", 0));
                var host =
                        new Host(
                                Map.of(
                                        EndpointSelection.Family.IPV4,
                                        (InetSocketAddress) stun.getLocalSocketAddress()),
                                List.of("8.8.8.8"))) {
            host.transport.installTicketKeys(KEYS).toCompletableFuture().get();
            var before = host.transport.captureHostProfile().toCompletableFuture().get();
            ...
            var positive =
                    new ProviderTransport.ConnectivityCheck(
                            ...
                            now + 100);
            host.transport
                    .reportConnectivityChecks(revision + 1, List.of(negative))
                    .toCompletableFuture()
                    .get();
            host.transport
                    .reportConnectivityChecks(revision, List.of(negative, positive))
                    .toCompletableFuture()
                    .get();
            assertEquals(
                    0,
                    host.transport.channel().nativeStats()[2],
                    "No monitor for old revision or positive direct check");
            ...
            assertThrows(
                    ...
                    () ->
                            host.transport
                                    .reportConnectivityChecks(revision, List.of(negative))
                                    .toCompletableFuture()
                                    .get());
            try {
                Thread.sleep(150);
            ...
            assertEquals(
                    0,
                    host.transport.channel().nativeStats()[2],
                    "Queued expired check cannot start a monitor");
            ...
            assertThrows(
                    ...
                    () -> stun.receive(new DatagramPacket(new byte[1024], 1024)),
                    "Direct failure cannot start STUN");
            var after = host.transport.captureHostProfile().toCompletableFuture().get();
            assertEquals(
                    revision,
                    after.candidateRevision(),
                    "Direct feedback cannot change host policy");
            assertEquals(Set.of(4, 6), after.assistedFamilies());
            assertEquals(
                    0,
                    after.profile().getAsJsonArray("candidates").size(),
                    "Failed direct endpoint is withheld without assistance");
