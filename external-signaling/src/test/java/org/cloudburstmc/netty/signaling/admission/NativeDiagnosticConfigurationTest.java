...
class NativeDiagnosticConfigurationTest {
    ...
    final Key diagnosticKey =
            new Key("K001", TestSignalingProvider.SECRET, 0, System.currentTimeMillis() + 300_000);
    ...
    final class Host implements AutoCloseable {
        ...
        Host(String address, boolean drained) throws Exception {
            ...
            port = port(bind);
            var fixture = new NativeDiagnosticHostTest();
            ...
            transport =
                    NativeProviderTransport.open(
                                    bootstrap,
                                    new InetSocketAddress(bind, port),
                                    advertised::get,
                                    directory.resolve("host.crt"),
                                    directory.resolve("host.key"),
            ...
            transport
                    .installTicketKeys(
                            List.of(
                                    new ProviderTransport.TicketKey(
        ...
        DiagnosticHostPolicy policy(long endpointExpiry) throws Exception {
            ...
            var endpoint =
                    new DiagnosticHostPolicy.Endpoint(
                            bind instanceof Inet6Address ? 6 : 4,
                            DiagnosticAdmissionCodec.address(
                                    bind instanceof Inet6Address ? 6 : 4, bind.getHostAddress()),
                            port,
                            snapshot.candidateRevision());
            return new DiagnosticHostPolicy(
                    context, List.of(diagnosticKey), Set.of(endpoint), endpointExpiry);
        }

        void configure(DiagnosticHostPolicy policy) throws Exception {
            transport.configureDiagnostics(policy).toCompletableFuture().get(5, TimeUnit.SECONDS);
        }

        Client client(long expiry) throws Exception {
            return new Client(bind, expiry);
        }
        ...
        void connect(Client client, DiagnosticHostPolicy policy) throws Exception {
            client.connect(
                    ...
                    port,
                    false,
                    policy.endpoints().iterator().next().candidateRevision());
        }
    ...
    @Test
    @Timeout(30)
    void bothFamiliesUseLocalConfigurationWithoutEnablingPlayers() throws Exception {
        for (String family : List.of("127.0.0.1", "::1")) {
            try (Host host = new Host(family)) {
                assertTrue(host.transport.supportsDiagnosticAdmission());
                ...
                var policy = host.policy(expiry + 1000);
                host.configure(policy);
                try (Client client = host.client(expiry)) {
                    host.connect(client, policy);
                    client.start(false);
                    var results = new ArrayList<NativeDiagnosticHostGate.Result>();
                    await(
                            () -> {
                                client.tick();
                                results.addAll(host.transport.pollDiagnosticResults(1));
                    ...
                    assertEquals(0, host.children.get());
                    assertEquals(0, host.transport.channel().creationAttempts());
                    assertEquals(0, host.transport.channel().liveNativePeers());
                    assertFalse(host.transport.channel().isServing());
                ...
                assertThrows(
                        ...
                        () -> host.transport.pollDiagnosticResults(33));
            }
        }
    }
    ...
    @Test
    @Timeout(25)
    void ordinaryTransportSupportsDiagnosticsWhileServing() throws Exception {
        for (String address : List.of("127.0.0.1", "::1")) {
            try (Host host = new Host(address, false)) {
                assertFalse(
                        host.transport.hostProfile().toCompletableFuture().get().has("version"));
    ...
    void ordinarySnapshotRevisionFencesRemapAbaAndDrain() throws Exception {
        for (String address : List.of("127.0.0.1", "::1")) {
            try (Host host = new Host(address, false)) {
                ...
                assertTrue(second.candidateRevision() > first.candidateRevision());
                assertThrows(
                        ProviderTransport.HostProfileSnapshotChangedException.class,
                ...
                assertThrows(
                        ...
                        () ->
                                host.transport
                                        ...
                                        .toCompletableFuture()
                                        .get());
                var current = host.policy(System.currentTimeMillis() + 20000);
                host.configure(current);
                host.transport.drain().toCompletableFuture().get();
                assertEquals(
                        IllegalStateException.class,
                        assertThrows(IllegalStateException.class, third::requireCurrent)
                                .getClass());
                assertThrows(
                        ExecutionException.class,
                        () ->
                                host.transport
                                        .configureDiagnostics(current)
                                        .toCompletableFuture()
                                        .get());
            }
        }
    ...
    @Test
    @Timeout(15)
    void queuedConfigurationUsesOriginalMonotonicDeadline() throws Exception {
            ...
            var pending =
                    host.transport
                            .configureDiagnostics(
                                    policy, System.nanoTime() + TimeUnit.MILLISECONDS.toNanos(100))
                            .toCompletableFuture();
            try {
                Thread.sleep(150);
            } finally {
    ...
    @Test
    @Timeout(25)
    void queuedConfigurationCannotSurviveDisableRemapOrClose() throws Exception {
        for (String change : List.of("disable", "remap", "close")) {
            try (Host host = new Host("127.0.0.1")) {
                ...
                var overCapacity =
                        host.transport.configureDiagnostics(policy).toCompletableFuture();
                assertThrows(ExecutionException.class, () -> overCapacity.get(1, TimeUnit.SECONDS));
                try {
                    switch (change) {
                        case "disable" -> {
                            host.transport.disableDiagnostics();
                        }
                        case "remap" -> {
                            host.remap(host.port + 1);
                        }
                        case "close" -> {
                ...
                assertThrows(
                        ExecutionException.class, () -> queued.get(5, TimeUnit.SECONDS), change);
                assertEquals(0, host.children.get());
            }
        }
    }
    ...
    @Test
    @Timeout(30)
    void withdrawalRemapAndContextChangesRetireDiagnosticsWithoutOpeningPlayers() throws Exception {
        for (String change : List.of("disable", "remap", "generation", "key")) {
            try (Host host = new Host("127.0.0.1")) {
                long expiry = (System.currentTimeMillis() + 15000) / 1000 * 1000;
                var policy = host.policy(expiry + 1000);
                host.configure(policy);
                try (Client client = host.client(expiry)) {
                    ...
                    switch (change) {
                        ...
                        case "remap" -> {
                            host.remap(host.port + 1);
                            host.remap(host.port);
                        }
                        case "generation" -> {
                            host.configure(
                                    new DiagnosticHostPolicy(
                                            new Context(
                                                    policy.context().providerOrigin(),
                                                    policy.context().hostId(),
                                                    policy.context().incarnation(),
                                                    2),
                                            policy.keys(),
                                            policy.endpoints(),
                                            policy.expiresAt()));
                    ...
                    assertFalse(results.get(0).success(), change);
                    assertTrue(results.get(0).cleanupComplete());
                    assertEquals(0, host.transport.channel().liveNativePeers());
                    assertFalse(host.transport.channel().isServing());
                    assertEquals(0, host.children.get());
    ...
    @Test
    @Timeout(25)
    void refreshNeverExtendsAnAdmittedAttempt() throws Exception {
        try (Host host = new Host("127.0.0.1")) {
            long expiry = (System.currentTimeMillis() + 3500) / 1000 * 1000;
            var policy = host.policy(expiry + 1000);
            host.configure(policy);
            try (Client client = host.client(expiry)) {
                host.connect(client, policy);
                await(() -> client.channels[0].isOpen() && client.channels[1].isOpen());
                host.configure(host.policy(expiry + 20000));
                var results = new ArrayList<NativeDiagnosticHostGate.Result>();
                await(
                        () -> {
                            results.addAll(host.transport.pollDiagnosticResults(1));
    ...
    @Test
    @Timeout(25)
    void familyDeadlinesAreIndependentAndPermitMustFitItsExactEndpoint() throws Exception {
        try (Host host = new Host("::1")) {
            long expiry = (System.currentTimeMillis() + 15000) / 1000 * 1000;
            var policy = host.policy(expiry + 1000);
            var six = policy.endpoints().iterator().next();
            var four =
                    new DiagnosticHostPolicy.Endpoint(
                            4, DiagnosticAdmissionCodec.address(4, "127.0.0.1"), host.port, 8);
                    ...
                    host.transport
                            ...
                            .enableDiagnostics(
                                    new DiagnosticHostPolicy(
                                            ...
                                            Set.of(four, six),
                                            policy.expiresAt(),
                                            Map.of(four, shortExpiry, six, policy.expiresAt())))
                            .toCompletableFuture()
                            .get();
            while (System.currentTimeMillis() <= shortExpiry) {
                Thread.sleep(10);
            }
            try (Client client = host.client(expiry)) {
                host.connect(client, policy);
                ...
                await(
                        () -> {
                            client.tick();
                            results.addAll(gate.pollResults());
                            return !results.isEmpty();
                        });
                assertTrue(results.get(0).success());
            }
            gate.replacePolicy(
                    new DiagnosticHostPolicy(
                            ...
                            Set.of(six),
                            policy.expiresAt(),
                            Map.of(six, System.currentTimeMillis() + 1000)));
            try (Client client = host.client(expiry)) {
                long rejected = gate.stats().rejected();
                host.connect(client, policy);
                await(() -> gate.stats().rejected() > rejected);
    ...
    @Test
    @Timeout(25)
    void foreignListenerAndEndpointsFailClosed() throws Exception {
        try (Host host = new Host("127.0.0.1")) {
            var policy = host.policy(System.currentTimeMillis() + 20000);
            var foreign =
                    new Context(
            ...
            var wrongEndpoint =
                    new DiagnosticHostPolicy.Endpoint(
                            4, DiagnosticAdmissionCodec.address(4, "127.0.0.2"), host.port, 7);
            for (var bad :
                    List.of(
                            new DiagnosticHostPolicy(
                                    foreign, policy.keys(), policy.endpoints(), policy.expiresAt()),
                            new DiagnosticHostPolicy(
                                    policy.context(),
                                    policy.keys(),
                                    Set.of(wrongEndpoint),
                                    policy.expiresAt()))) {
                assertThrows(
                        ExecutionException.class,
                        () ->
                                host.transport
                                        .configureDiagnostics(bad)
                                        .toCompletableFuture()
    ...
    @Test
    @Timeout(25)
    void localConfigurationAndWithdrawalPreserveExistingPlayer() throws Exception {
        try (Host host = new Host("127.0.0.1", false);
                PeerConnection player =
                        PeerConnection.createPeer(
                                PeerConnectionConfiguration.DEFAULT
                                        .withBindAddress(InetAddress.getLoopbackAddress())
            ...
            var answer =
                    TestSignalingProvider.answer(
                            ...
                            host.port,
                            expiry,
                            NativeProviderTransport.audience(
                                    host.transport
                                            .hostProfile()
                                            .toCompletableFuture()
                                            .get()
            ...
            host.transport.disableDiagnostics().toCompletableFuture().get();
            assertTrue(host.transport.channel().isServing());
            assertEquals(1, host.transport.channel().liveNativePeers());
            assertEquals(1, host.children.get());
            reliable.sendMessage(ByteBuffer.allocateDirect(2).put((byte) 0).put((byte) 42).flip());
            await(() -> echoes.get() == 2);
