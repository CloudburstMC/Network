...
class NativeConnectivityIntegrationTest {
    ...
    private static void respondTwice(DatagramSocket server, int gameplayPort) throws Exception {
        ...
        for (int n = 0; n < 2; n++) {
            ...
            assertEquals(
                    gameplayPort,
                    packet.getPort(),
                    "Monitor must use the gameplay mux source port");
            ...
            response.putInt(0x2112a442)
                    .put(transaction)
                    .putShort((short) 0x20)
                    .putShort((short) (address.length + 4));
            response.put((byte) 0)
                    .put((byte) (address.length == 4 ? 1 : 2))
                    .putShort((short) (packet.getPort() ^ 0x2112));
            for (int i = 0; i < address.length; i++) {
                response.put((byte) (address[i] ^ request[4 + i]));
            }
            server.send(
                    new DatagramPacket(
                            response.array(), response.capacity(), packet.getSocketAddress()));
        }
    }
    ...
    void sameDualStackGameplayMuxRefreshesWithoutPlayersAndClosesWithChannel() throws Exception {
        ...
        int port;
        try (var unused = new DatagramSocket(new InetSocketAddress(v6, 0))) {
        ...
        try (var stun4 = new DatagramSocket(new InetSocketAddress(v4, 0));
            ...
            host =
                    NativeProviderTransport.open(
                                    ...
                                    bind,
                                    new InetSocketAddress(v4, port),
                                    identity.certificate(),
                                    identity.privateKey(),
                                    AdmissionGate.Limits.defaults())
                            .toCompletableFuture()
                            .get(5, TimeUnit.SECONDS);
            host.installTicketKeys(
                            List.of(
            ...
            var controller =
                    channel.enableConnectivity(selection, servers, Duration.ofSeconds(30))
                            .toCompletableFuture()
                            .get();
            assertThrows(
                    ExecutionException.class,
            ...
            Future<?> first =
                    responders.submit(
                            () -> {
                                respondTwice(stun4, port);
                                return null;
                            });
            Future<?> second =
                    responders.submit(
                            () -> {
                                respondTwice(stun6, port);
                                return null;
                            });
            controller.snapshot();
            ...
            do {
                ...
                if (snapshot.families().values().stream()
                        .allMatch(
                                f ->
                                        f.observation().isPresent()
                                                && f.observation().get().successfulResponses()
                                                        >= 2)) {
                    break;
                }
                Thread.sleep(10);
            ...
            for (var family : Family.values()) {
                ...
                assertTrue(
                        state.freshStunEndpoint().isEmpty(),
                        "Loopback mapping must never enter a public candidate profile");
            ...
            assertEquals(
                    2,
                    channel.nativeStats()[2],
                    "Two monitor agents share the existing mux with no player agents");
            assertEquals(0, channel.creationAttempts());
            assertEquals(0, channel.nativeStats()[3]);
            assertEquals(0, channel.nativeStats()[5]);
            assertEquals(
                    profile,
                    host.hostProfile().toCompletableFuture().get(),
                    "Monitoring cannot mutate admission incarnation/profile");
            host.close().toCompletableFuture().get(5, TimeUnit.SECONDS);
            assertTrue(
                    controller.snapshot().families().values().stream()
                            .allMatch(
                                    f -> f.state() == EndpointConnectivityController.State.CLOSED));
            try (var reused = new DatagramSocket(bind)) {
                assertEquals(port, reused.getLocalPort());
            }
            System.out.println(
                    "native-connectivity PASS dualStack=true sameGameplayPort=true responsesPerFamily=2 playerCreations=0 channelCloseReleasesMonitors=true");
        } finally {
            if (host != null) {
                host.close().toCompletableFuture().get(5, TimeUnit.SECONDS);
            }
