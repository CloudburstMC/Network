...
class NativeAssistedAdmissionTest {
    ...
    void timedOutPrecreatedPeersReleaseAssistedCapacityAfterNativeCleanup() throws Exception {
        var identityHelper = new NativeDiagnosticHostTest();
        identityHelper.directory = directory;
        ...
        var endpoint =
                new NativeAdmissionServerChannel(
                        identity, (request, now) -> null, new AdmissionGate.Limits(2, 64, 1, 100));
        try {
            new ServerBootstrap()
                    .group(loop)
                    ...
                    .bind(bind, NativeDiagnosticHostTest.port(bind))
                    .sync();
            try (var client =
                ...
                for (int attempt = 0; attempt < 33; attempt++) {
                    var join =
                            new AssistedJoin(
                                    ...
                                    "1234",
                                    TestSignalingProvider.IDENTITY_CPK,
                                    "timeoutHost" + attempt,
                                    "h".repeat(32),
                                    client.localDescription());
                    endpoint.assist(join, () -> {}).toCompletableFuture().get(5, TimeUnit.SECONDS);
                    // Deliberately withhold the answer: no peer can reach activation or consume the
                    // CPK.
                ...
                assertEquals(
                        ...
                        "Timeouts cannot permanently consume the32 assisted slots");
    ...
    void assistedPeerUsesRealGameplayChildIdentityAndTwoWayChannelsBothFamilies() throws Exception {
                ...
                KeyFactory.getInstance("EC")
                        .generatePublic(
                                new X509EncodedKeySpec(
                                        Base64.getDecoder()
                                                .decode(TestSignalingProvider.IDENTITY_CPK)));
        for (String numeric : List.of("127.0.0.1", "::1")) {
            ...
            var endpoint =
                    new NativeAdmissionServerChannel(
                            identity, (r, now) -> null, new AdmissionGate.Limits(4, 8, 2, 15000));
            ...
            try {
                ...
                try (var client =
                    ...
                    for (int i = 0; i < 2; i++) {
                        ...
                        channels[i].onMessage.register(
                                new DataChannelCallback.Message() {
                                    public void onText(DataChannel dc, String s) {
                                        errors.add(new AssertionError("binary required"));
                                    }

                                    public void onBinary(DataChannel dc, ByteBuffer bytes) {
                                        assertEquals(0, bytes.get());
                                        echoes.add(dc.label() + ":" + bytes.get());
                                    }
                                });
                    }
                    client.setLocalDescription(
                            "offer",
                            "assistedPlayer",
                            "p".repeat(128)); // Stateless 91-byte bound remains unchanged.
                    assertTrue(gathered.await(5, TimeUnit.SECONDS));
                    // The first offered tuple is deliberately not the eventual peer. Reporting the
                    // admission reservation instead of the selected native endpoint would emit
                    // port9.
                    String offer =
                            client.localDescription()
                                    .replaceFirst(
                                            "a=candidate:",
                                            "a=candidate:decoy 1 UDP 1 "
                                                    + numeric
                                                    + " 9 typ host\r\na=candidate:");
                    var join =
                            new AssistedJoin(
                                    ...
                                    offer);
                    String answer =
                            endpoint.assist(join, () -> {})
                                    .toCompletableFuture()
                                    .get(5, TimeUnit.SECONDS);
                    assertEquals(
                            1,
                            endpoint.creationAttempts(),
                            "peer exists before client receives an answer");
                    assertThrows(
                            ExecutionException.class,
                            () -> endpoint.assist(join, () -> {}).toCompletableFuture().get());
                    assertEquals(1, endpoint.creationAttempts());
                    client.setRemoteDescription(
                            answer.replaceAll("(?m)^a=candidate:[^\\r\\n]*\\r?\\n", "")
                    ...
                    assertEquals(
                            Set.of("ReliableDataChannel:60", "UnreliableDataChannel:61"),
                            Set.of(
                                    echoes.poll(5, TimeUnit.SECONDS),
                                    echoes.poll(5, TimeUnit.SECONDS)));
                    ...
                    NativeDiagnosticHostTest.await(
                            () -> {
                                endpoint.pollEvents().stream()
                                        .filter(e -> e.stage().equals("ticket.data_channels_open"))
                                        .forEach(observed::set);
                                return observed.get() != null;
                            });
                    var selected = client.selectedCandidatePair().local();
                    assertEquals(
                            new java.net.InetSocketAddress(bind, selected.getPort()),
                            observed.get().remoteEndpoint());
                    assertNotEquals(9, observed.get().remoteEndpoint().getPort());
                    assertTrue(client.closeAndAwait(Duration.ofSeconds(5)));
                    assertEquals(
                            selected.getPort(),
                            observed.get().remoteEndpoint().getPort(),
                            "Original observation survives closure");
                }
            } finally {
                endpoint.close().awaitUninterruptibly();
                endpoint.termination().toCompletableFuture().get(6, TimeUnit.SECONDS);
