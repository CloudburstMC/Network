...
class NativeDiagnosticProbeAttemptTest {
    ...
    @Test @Timeout(25) void bothFamiliesCompleteWithIndependentHostDigestAndActualCleanup() throws Exception {
        for(String address:List.of("127.0.0.1","::1"))try(Fixture fixture=new Fixture(address,20000);var attempt=fixture.attempt()) {
            ...
            assertTrue(result.answerVerified());assertTrue(result.transportConnected());assertTrue(result.authSent());assertTrue(result.exchangeComplete());assertTrue(result.cleanupComplete());
            assertNotNull(result.completionDigestHex());assertEquals(fixture.job,result.job());assertEquals(fixture.localPort,result.selectedLocal().getPort());assertEquals(fixture.port,result.selectedRemote().getPort());
            assertEquals(fixture.bind,result.selectedLocal().getAddress());assertEquals(fixture.bind,result.selectedRemote().getAddress());
            assertEquals(6,result.sentFrames());assertEquals(5,result.receivedFrames());assertTrue(result.udp().sentDatagrams()>2);assertEquals(0,result.udp().rejectedDatagrams());assertTrue(result.udp().reservedDatagrams()<=MAX_UDP_SENDS);
            ...
            assertEquals(1,reports.size());assertTrue(reports.get(0).success(),reports.toString());assertEquals(result.completionDigestHex(),reports.get(0).completionDigestHex());
    ...
    @Test @Timeout(25) void providerAnswerFailuresSendNoPeerPacketsOrAdmission() throws Exception {
        for(String mode:List.of("signature","pin","oversize","catalog","withdraw"))try(Fixture fixture=new Fixture("127.0.0.1",15000);var attempt=fixture.attempt()) {
            ...
            assertFalse(result.success(),mode);assertFalse(result.answerVerified(),mode);assertFalse(result.transportConnected(),mode);assertTrue(result.cleanupComplete(),mode);assertNull(result.completionDigestHex(),mode);
    ...
    @Test @Timeout(20) void installedHostWithdrawalAfterAnswerCannotQualify() throws Exception {
        try(Fixture fixture=new Fixture("::1",4000);var attempt=fixture.attempt()) {
            ...
            assertFalse(result.success());assertTrue(result.answerVerified());assertFalse(result.authSent());assertTrue(result.cleanupComplete());assertNull(result.completionDigestHex());assertTrue(result.udp().sentDatagrams()>0);assertEquals(0,fixture.gate.stats().liveNativePeers());assertEquals(0,fixture.players.get());
        }
    }
    @Test @Timeout(20) void signedButWrongActualDtlsIdentityFailsBothFamilies() throws Exception {
        for(String address:List.of("127.0.0.1","::1"))try(Fixture fixture=new Fixture(address,4000)) {
            ...
            try(var attempt=new NativeDiagnosticProbeAttempt(expected,new InetSocketAddress(fixture.bind,fixture.localPort),fixture.catalog::get,fixture.authorized::get,Clock.system(),true)) {
                ...
                assertFalse(result.success(),result.toString());assertTrue(result.answerVerified());assertFalse(result.transportConnected());assertFalse(result.authSent());assertFalse(result.exchangeComplete());assertNull(result.completionDigestHex());assertTrue(result.cleanupComplete());
    ...
    @Test @Timeout(20) void clockCorrectionOrCancellationAfterNativeChecksCannotCreateSuccess() throws Exception {
        for(String change:List.of("forward wall","backward wall with elapsed monotonic","withdraw after checks"))try(Fixture fixture=new Fixture("127.0.0.1",10000)) {
            ...
            try(var attempt=new NativeDiagnosticProbeAttempt(fixture.job,new InetSocketAddress(fixture.bind,fixture.localPort),fixture.catalog::get,authorized,clock,true)) {
                ...
                assertFalse(result.success(),change);assertNull(result.completionDigestHex(),change);assertTrue(result.cleanupComplete(),change);
