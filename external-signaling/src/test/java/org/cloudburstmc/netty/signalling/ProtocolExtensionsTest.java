...
import org.cloudburstmc.netty.signaling.ProtocolExtensions;
...
class ProtocolExtensionsTest {
    ...
    @Test
    void ignoresUnknownOptionalVersionsWithoutGrantingCoreCapabilities() {
        ...
        JsonObject copy = ProtocolExtensions.copy(input);
        copy.remove("org.example.feature");
        assertEquals(1, input.getAsJsonObject("extensions").size());
    }

    @Test
    void rejectsRequiredOrMalformedExtensions() {
    ...
    @Test
    void boundsEncodedSizeAndCount() {
        ...
        for (int i = 0; i < 17; i++) {
            value.getAsJsonObject("extensions").add("org.example.feature" + i, extension.deepCopy());
        }
        assertThrows(IllegalArgumentException.class, () -> ProtocolExtensions.validate(value));
    }
}
