...
This opt-in contract carries an immutable native host observation. The codec and pure Warden correlator are staged primitives: they do not mount an upload route, drain a native queue, create a receipt, enroll a target, schedule a probe, publish a report or change player readiness. Authentication of the host and workload, original issuance history, durable idempotent storage and final delivery guards remain integration requirements.

The wire schema and independent Python-generated public vectors are in `control-v1.diagnostic-completion.schema.json` and `control-v1.diagnostic-completion.fixtures.json`. Network must mirror these new files before the shared-contract provenance inventory is advanced. The original installation binding uses `control-v1.diagnostic-installation.schema.json` without changing its grammar.
...
The existing Network `NativeDiagnosticHostGate.Result` and neutral `DiagnosticAdmission.Completion` preserve these fields. A future emitter must use the admission-time installation capture and check that its separate native `context` equals the binding's provider/host/native incarnation/generation. A null installation from an unassociated low-level fixture cannot be uploaded through this contract. Do not rebind a drained result to today's installation or reconstruct it from a mutable profile. Do not infer direction counts from aggregate frame counters: `DiagnosticExchange.complete()` and its digest are the host's four-direction proof.
...
The independent batch format is `{version:1,completions:[...]}` with **1..4** entries and **16384 UTF-8 bytes** total. Repeated provider/host/authority incarnation/generation/native owner tuple/attempt identities are rejected even if their contents differ. Historical cross-owner identities remain distinct. This batch is not yet a heartbeat field. Future polling/durable queues retain the delivery plan's native32, poll4 and local durable32 bounds, exact receipt reconciliation, and explicit unknown/loss accounting at capacity.
...
- Host completion plus immutable completion digest, authenticated host ID and original recordedAt, supplied by the future authenticated host-result store.
...
- Original trusted `admissionKeyId`. Current AttemptRecord does not persist the offer's selected epoch ID; later issuance/store integration must supply this original association. Missing key history is unknown, not guessed from a rotated installation.
