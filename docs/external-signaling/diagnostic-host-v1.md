...
Diagnostics use ordinary ICE peers. Profile 1 validates one exact selected destination. Profile 2 starts with the authenticated peer candidate from the signed offer and permits same-family public peer-reflexive endpoints learned through authenticated ICE checks. A private IPv4 host candidate in the signed offer is accepted only as client-family information: remove it from native remote SDP and wait for authenticated incoming ICE to learn the public source. Never send diagnostic traffic to the offered private address. The probe applies one signed fresh public host candidate and starts ICE toward it. A shared per-attempt gatherer serves player and diagnostic assistance; its STUN monitor stops at the original deadline or completion, without restarting background warming. Attempts last at
