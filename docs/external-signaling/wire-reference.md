...
helper handles both built-in signaling and NXS. Install its channel evidence
...
signaling key. Preserve that existing contract: after validating its forwarded
...
client is responsible for the equivalent signaling/login comparison there.
...
Run `node docs/external-signaling/fixtures.mjs` for independent signature and
