...
Both issuer and verifier check the actual answer against the closed profile: one UDP/DTLS/SCTP application/mid 0, BUNDLE 0, full ICE, setup `active` (matching the existing NXS answer builder), SCTP 5000/max-message-size 262144, completed gathering and, for profile 1, one numeric target candidate whose family/address/port exactly matches the authorized request. Profile 2 uses a zero-address/zero-port request target and requires one freshly gathered same-family host or server-reflexive answer candidate, bound by the provider signature. The runner checks public scope before applying it; both peers can then send ICE to open restrictive NAT filters. The host gathers per attempt without renewing a failed background warm mapping. `host` and `srflx` are allowed; related-address metadata is bounded numeric metadata, not an alternate destination. Unknown candidate types, relay/TCP, invalid media ports, ambiguous/indented duplicate attributes, player identity markers and NXS1 player credentials are rejected. The NXD1 username and 32-character password must have the expected carrier shape. Without a host secret the answer verifier cannot decrypt that capability; the provider signature authenticates its exact bytes.

Candidate type is the provider's authenticated assertion. Actual selected candidate pair, transport family, pinned DTLS, channel labels/reliability and any optional ping/pong still require native runtime observation. An SDP candidate or a valid signature is not reachability evidence.
