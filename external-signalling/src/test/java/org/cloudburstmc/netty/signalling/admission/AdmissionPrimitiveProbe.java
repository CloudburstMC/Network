...
public final class AdmissionPrimitiveProbe {
    ...
    public static void main(String[] args) throws Exception {
        ...
        for (int passwordLength : new int[]{24, 32, 91}) run(identity, passwordLength, false);
    ...
    static void run(NativeHostIdentity identity, int passwordLength, boolean wrongFingerprint) throws Exception {
        ...
        try (var client = PeerConnection.createPeer(PeerConnectionConfiguration.DEFAULT.withDisableAutoNegotiation(true).withBindAddress(LOOPBACK), Runnable::run)) {
                ...
                .childHandler(new ChannelInitializer<AdmittedNetherNetChildChannel>() {
                    @Override protected void initChannel(AdmittedNetherNetChildChannel child) {
                        child.pipeline().addLast(new SimpleChannelInboundHandler<ByteBuf>() {
                            ...
                            @Override protected void channelRead0(ChannelHandlerContext ctx, ByteBuf message) {
                                int bit = reliable ? 1 : 2;
                                check(message.readableBytes() == 1 && message.readByte() == bit, "channel identity and payload");
                                received.getAndUpdate(mask -> mask | bit); messages.countDown();
                            }
            ...
            for (boolean reliable : new boolean[]{true, false}) {
                var channel = client.createDataChannel(reliable ? "ReliableDataChannel" : "UnreliableDataChannel",
                    DataChannelInitSettings.DEFAULT.withReliability(new DataChannelReliability(!reliable, !reliable, 0, 0)));
                channel.onOpen.register(dc -> dc.sendMessage(ByteBuffer.allocateDirect(2).put((byte)0).put((byte)(reliable ? 1 : 2)).flip()));
            }
            ...
            if (wrongFingerprint) {
                ...
                while (!rejected && System.nanoTime() < deadline) {
                    ...
                    if (!rejected) Thread.sleep(10);
                }
                check(rejected && received.get() == 0, "DTLS rejects a client certificate that does not match its token");
            } else {
            ...
            check(endpoint.creationAttempts() == 1 && endpoint.nativeStats()[5] == 1, "one peer and one admission notification");
            System.out.println("native-admission PASS ufragChars=" + answer.token().length() + " wrongClientFingerprint=" + wrongFingerprint + " channels=" + received.get() + " admissionNotifications=1");
        } finally {
